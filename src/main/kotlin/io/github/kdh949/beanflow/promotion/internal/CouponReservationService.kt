...
internal class CouponReservationService(
	...
	override fun reserve(command: ReserveCouponCommand): CouponReservationQuote {
		...
		val issuance = issuanceRepository.findLockedById(command.couponIssuanceId)
		...
		if (issuance.customerId != command.customerId || issuance.state != CouponIssuanceState.AVAILABLE ||
		...
		val campaign = campaignRepository.findById(issuance.campaignId).orElse(null)
		...
		val targetMenus = if (campaign.allMenusEligible) {
			null
		} else {
		...
		val eligibleSubtotal = eligibleLines.fold(0L) { total, line ->
		...
		val reservation = CouponReservationEntity(
	...
	private fun calculateDiscount(campaign: CampaignEntity, eligibleSubtotal: Long): Long =
		when (campaign.discountType) {
			...
			CouponDiscountType.RATE_BPS -> {
				...
				val raw = BigInteger.valueOf(eligibleSubtotal)
	...
	private fun CouponReservationEntity.toQuote(): CouponReservationQuote =
		CouponReservationQuote(
			...
			eligibleLineSequences = eligibleLineSequences
	...
	private fun invalidCampaign(): Nothing =
		fail(FailureCode.COUPON_NOT_AVAILABLE, "Coupon campaign discount fields are invalid")
