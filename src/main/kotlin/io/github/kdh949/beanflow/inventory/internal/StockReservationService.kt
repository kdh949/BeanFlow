...
internal class StockReservationService(
	...
	override fun reserve(command: ReserveStockCommand): List<UUID> {
		...
		for (requirement in requirements) {
			val stock = stockRepository.findLockedById(requirement.sellableUnitId)
			...
			val existing = reservationRepository.findBySourceReferenceAndSellableUnitId(
	...
	private fun aggregate(requirements: List<StockRequirement>): List<StockRequirement> =
		requirements.groupBy(StockRequirement::sellableUnitId)
			.map { (id, values) ->
				val quantity = values.fold(0L) { total, requirement ->
