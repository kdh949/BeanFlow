...
internal class SupportCompensationApplicationService(
    ...
    fun evaluate(command: EvaluateSupportCompensationCommand): SupportCompensationEvaluationResource =
        transactions.evaluate(command, command.orderId?.let(ordering::find))

    fun create(command: CreateSupportCompensationCommand): SupportCompensationResource =
        transactions.create(command, command.orderId?.let(ordering::find))

    fun execute(command: ExecuteSupportCompensationCommand): SupportCompensationResource {
        ...
        return requestNotification(issued, command.actorId, null)
    }
    ...
    fun retryNotification(command: RetrySupportCompensationNotificationCommand): SupportCompensationResource {
        ...
        return requestNotification(resource, command.actorId, command)
    }
    ...
    private fun requestNotification(
        ...
        if (resource.state == SupportCompensationRequestState.NOTIFICATION_ACCEPTED) return get(actorId, resource.compensationRequestId)
        val accepted =
            try {
        ...
        return get(actorId, resource.compensationRequestId)
    }
