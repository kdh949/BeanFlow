...
internal data class OperatorStorefrontImageAuthoringResponse(
    val image: OperatorStorefrontImageResponse?,
)
    ...
    fun current(
        ...
        ResponseEntity.ok().cacheControl(CacheControl.noStore()).body(
            OperatorStorefrontImageAuthoringResponse(
                service.current(actorId(actor), storeId, reason)?.let(OperatorStorefrontImageResponse::of),
            ),
        )
