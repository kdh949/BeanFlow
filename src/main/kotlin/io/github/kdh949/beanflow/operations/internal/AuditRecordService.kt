...
internal class AuditRecordService(
	...
	override fun appendAll(commands: List<AppendAuditRecordCommand>): List<UUID> {
		...
		val records = commands.map { command ->
			validate(command)
			AuditRecordEntity(
	...
	internal fun retentionExpiry(occurredAt: Instant): Instant =
		occurredAt.atZone(SEOUL).plusYears(5).toInstant()

	@Transactional
	fun purgeDue(now: Instant, chunkSize: Int): AuditRetentionPurgeResult {
		require(chunkSize > 0)
