...
internal class PaymentCancellationSetupIntegrityQueryService(
    ...
    @Transactional(readOnly = true)
    fun findCurrentCancellationVersion(orderId: UUID): Long? =
        jdbcTemplate
            .query(
                "SELECT version FROM ordering_order WHERE id = ? AND state = 'CANCELLED' " +
    ...
    @Transactional(readOnly = true)
    fun oldestOpenCaseCreatedAt(): Instant? =
        jdbcTemplate.queryForObject(
            "SELECT min(created_at) FROM operations_reprocessing_case " +
    ...
    private companion object {
        val ASSESSMENT_SQL =
            """
            SELECT COALESCE(snapshot.correlation_id, source_refund.correlation_id, payment.correlation_id,
                            'setup-integrity:' || customer_order.id) AS correlation_id,
                   snapshot.id IS NULL AS snapshot_missing,
                   (snapshot.id IS NULL AND source_refund.id IS NULL)
                       OR (snapshot.id IS NOT NULL
                           AND snapshot.cancellation_requested_refund_amount_krw > 0
                           AND refund.id IS NULL) AS refund_missing,
                   CASE
                       WHEN snapshot.id IS NULL THEN source_refund.id IS NOT NULL AND (
                            source_refund.order_id <> customer_order.id
                            OR source_refund.payment_id <> payment.id
                            OR source_refund.reason <> 'CUSTOMER_ORDER_CANCELLED')
              ...
              LEFT JOIN payment_refund source_refund
                ON source_refund.source_reference =
                   'order:' || customer_order.id || ':customer-cancellation:' || customer_order.version || ':payment'
        ...
        val SCAN_SQL =
            ...
            SELECT customer_order.id AS order_id,
              ...
              LEFT JOIN payment_cancellation_recovery_snapshot snapshot
                ON snapshot.order_id = customer_order.id
              LEFT JOIN payment_refund refund ON refund.id = snapshot.cancellation_refund_id
             WHERE customer_order.state = 'CANCELLED'
               ...
               AND (
                    snapshot.id IS NULL
                    OR (snapshot.cancellation_requested_refund_amount_krw > 0 AND refund.id IS NULL)
                    OR snapshot.order_id <> customer_order.id
                    OR snapshot.cancellation_order_version <> customer_order.version
                    OR snapshot.payment_id <> payment.id
                    OR (snapshot.cancellation_requested_refund_amount_krw = 0 AND (
                           snapshot.cancellation_refund_id IS NOT NULL
                           OR snapshot.refund_source_reference IS NOT NULL
