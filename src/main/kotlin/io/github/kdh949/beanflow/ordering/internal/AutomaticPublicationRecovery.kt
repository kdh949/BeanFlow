...
internal class AutomaticPublicationRecoveryScope {
    ...
    private val workerRun = ThreadLocal<WorkerRun>()
    private val dataRead = ThreadLocal.withInitial { false }
    ...
    fun dataReadSucceeded() {
        workerRun.get()?.dataReadSucceeded()
        dataRead.set(true)
    }

    fun itemClaimed(): Boolean =
        workerRun.get()?.let {
            it.claimed()
            true
        } ?: false
    ...
    fun itemOutcome(
        ...
        workerRun.get()?.let {
            when (outcome) {
                WorkerItemOutcome.COMPLETED -> it.completedAfter(duration)
                WorkerItemOutcome.FAILED -> it.failedAfter(duration)
            }
            true
        } ?: false
    ...
    fun <T> run(
        ...
        current.set(now)
        run?.let(workerRun::set)
        dataRead.set(false)
        try {
            return action().also {
                if (!dataRead.get()) run?.dataReadSucceeded()
            }
        } finally {
            current.remove()
            candidates.remove()
...
internal class AutomaticPublicationRecoveryRepository(
    ...
    private val automaticClaims = ConcurrentHashMap<UUID, Long>()
    ...
    override fun markFailed(identifier: UUID) {
        ...
        if (selected == null) {
            ...
            recordAutomaticOutcome(identifier, WorkerItemOutcome.FAILED)
        } else {
            execution.result(selected, "FAILED") { delegate.markFailed(identifier) }
        }
    }
    ...
    override fun markResubmitted(
        ...
        scope.selection(identifier)?.let {
            return execution.claim(it, resubmissionDate).also { claimed ->
                if (claimed) {
                    automaticClaims[identifier] = System.nanoTime()
                    if (!scope.itemClaimed()) {
                        workerTelemetry.recordClaimed(WorkerOwner.EVENT_PUBLICATION)
                    }
                }
            }
        }
    ...
    override fun markCompleted(
        ...
        if (selected == null) {
            ...
            recordAutomaticOutcome(publication.identifier, WorkerItemOutcome.COMPLETED)
        } else {
    ...
    override fun findFailedPublications(criteria: EventPublicationRepository.FailedCriteria): List<TargetEventPublication> {
        ...
        return queries.findDue(now, criteria).also {
            scope.remember(it)
            scope.dataReadSucceeded()
        }
    }

    private fun recordAutomaticOutcome(
        identifier: UUID,
        outcome: WorkerItemOutcome,
    ) {
        val claimedAt = automaticClaims.remove(identifier) ?: return
        val duration = Duration.ofNanos(System.nanoTime() - claimedAt)
        if (!scope.itemOutcome(outcome, duration)) {
            workerTelemetry.recordOutcome(WorkerOwner.EVENT_PUBLICATION, outcome, duration)
        }
    }
}
