...
internal class ReservationExpiryService(
	...
	@Transactional
	override fun expireIfDue(orderId: UUID, now: Instant): ReservationExpiryResult {
		try {
			...
			val coupon = if (order.couponDiscountKrw > 0) {
				requireApplied(
					"COUPON",
			...
			val points = if (order.pointsAppliedKrw > 0) {
				requireApplied(
					"POINTS",
			...
			meterRegistry.summary("beanflow.reservation.expiry.lag.seconds")
				.record(java.time.Duration.between(deadline, now).toMillis().coerceAtLeast(0) / 1000.0)
			logger.info(
	...
	private fun dependencyFailure(message: String) =
		DomainFailure(FailureCode.DEPENDENCY_UNAVAILABLE, message)
