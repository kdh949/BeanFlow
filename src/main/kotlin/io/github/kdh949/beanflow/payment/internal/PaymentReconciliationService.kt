...
internal class PaymentReconciliationService(
	...
	@Transactional
	override fun claimDue(now: Instant, limit: Int): List<ClaimedPaymentReconciliation> {
		...
		return reconciliationRepository.findDueIds(now, PageRequest.of(0, limit)).mapNotNull { workId ->
			...
			val payment = paymentRepository.findById(work.paymentId).orElse(null)
	...
	override fun requestProviderLookup(paymentId: UUID): ProviderPaymentResult =
		gateway.lookup(requestLoader.loadLookup(paymentId))
	...
	override fun recordUnknown(
		...
		val entity = paymentRepository.findLockedById(work.paymentId)
			?: dependencyFailure("Payment for reconciliation is missing")
		...
		val idempotency = idempotencyRepository.findByPaymentId(work.paymentId)
		...
		if (nextAttempt >= MAX_ATTEMPTS) {
			...
			idempotency.responseBody = paymentBody(entity, "MANUAL_REVIEW", "MANUAL_REVIEW", now)
			...
			idempotency.status = if (entity.approvalState == PaymentApprovalState.RECONCILING) {
	...
	override fun recordRecovery(
		...
		when {
			result.outcome == ProviderRecoveryOutcome.SUCCEEDED -> {
				...
				completeRecoveryIdempotency(entity, locked, now)
			}
			...
			locked.attemptCount >= MAX_ATTEMPTS -> {
				...
				entity.approvalState = PaymentApprovalState.MANUAL_REVIEW
				entity.updatedAt = now
				val idempotency = idempotencyRepository.findByPaymentId(entity.id)
					?: dependencyFailure("Payment idempotency record is missing")
				idempotency.status = PaymentIdempotencyStatus.MANUAL_REVIEW
	...
	private fun completeRecoveryIdempotency(
		...
		val idempotency = idempotencyRepository.findByPaymentId(payment.id)
			?: dependencyFailure("Payment idempotency record is missing")
		idempotency.status = PaymentIdempotencyStatus.COMPLETED
		idempotency.responseStatus = 200
		idempotency.responseBody = paymentBody(payment, "RECONCILING", "SUCCEEDED", now)
		idempotency.terminalAt = now
		payment.updatedAt = now
		work.updatedAt = now
	}

	private fun scheduleRefund(payment: PaymentEntity, now: Instant) {
	...
	private fun lockClaim(claim: ClaimedPaymentReconciliation): PaymentReconciliationEntity {
		val work = reconciliationRepository.findLockedById(claim.workId)
	...
	private fun clearClaim(work: PaymentReconciliationEntity, now: Instant) {
	...
	private fun paymentBody(
		...
		objectMapper.writeValueAsString(
			mapOf(
				...
				"approvedAmountKrw" to payment.approvedAmountKrw,
				"currency" to payment.currency,
				"recovery" to mapOf("state" to recoveryState, "lastUpdatedAt" to now),
				"updatedAt" to now,
	...
	private fun PaymentEntity.toReconciliationView(work: PaymentReconciliationEntity) =
		ExternalPaymentView(
			...
			currency = currency,
			recoveryState = when (work.status) {
	...
	private fun ReconciliationKind.toApi(): PaymentReconciliationWorkKind =
		PaymentReconciliationWorkKind.valueOf(name)

	private fun normalized(code: String): String =
		code.trim().uppercase().replace(Regex("[^A-Z0-9_]+"), "_").take(80).ifBlank { "UNKNOWN" }

	private fun dependencyFailure(message: String): Nothing =
