...
internal class ExternalPaymentService(
    ...
    override fun existing(command: PrepareExternalPaymentCommand): PaymentPreparation? {
        validate(command)
        val existing =
            idempotencyRepository.findByActorIdAndOperationAndIdempotencyKey(
                command.actorId,
                OPERATION,
    ...
    override fun prepare(command: PrepareExternalPaymentCommand): PaymentPreparation {
        ...
        val paymentMethod =
            paymentMethodRepository.findLockedById(command.paymentMethodId)
    ...
    override fun applyResult(command: ApplyExternalPaymentResultCommand): ExternalPaymentView {
        ...
        if (command.lateApproval) {
            val approved =
                command.result as? ProviderPaymentResult.Approved
        ...
        val work =
            reconciliationRepository.findByPaymentIdAndKind(
                command.paymentId,
        ...
        when (payment.approvalState) {
            ...
            PaymentApprovalState.RECONCILING -> {
                ...
                fail(FailureCode.ORDER_STATE_CONFLICT, "Unexpected payment result state")
            }
        }
    ...
    private fun PaymentIdempotencyEntity.toPreparation(command: PrepareExternalPaymentCommand): PaymentPreparation {
    ...
    private fun ProviderPaymentResult.toDomain(): ProviderApproval =
        when (this) {
            ...
            is ProviderPaymentResult.Declined -> {
                ProviderApproval.Declined(code)
            }

            is ProviderPaymentResult.Unknown -> {
                ProviderApproval.Unknown(code)
            }
        }
    ...
    internal companion object {
        ...
        val RECONCILIATION_DELAYS: List<Duration> =
            listOf(
...
internal class PaymentProviderRequestLoader(
    ...
    private fun snapshot(payment: PaymentEntity): PaymentProviderRequestSnapshotEntity {
        val snapshot =
            snapshots.findByPaymentId(payment.id)
...
private fun PaymentIdempotencyJpaRepository.findByIdempotencyPaymentId(paymentId: UUID): PaymentIdempotencyEntity =
