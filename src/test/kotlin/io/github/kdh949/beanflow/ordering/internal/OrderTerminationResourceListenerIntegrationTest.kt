...
internal class OrderTerminationResourceListenerIntegrationTest
    ...
    constructor(
        ...
        @Test
        fun `OrderCancelledV1 releases pickup with terminal version sources`() {
            ...
            await("cancellation resource listeners") {
                pickupReservationRepository.findByOrderId(orderId)?.state ==
                    ...
                    compensationOperations
                        ...
                        ?.steps
                        ?.filter { it.type == OrderCompensationStepType.PICKUP }
                        ?.let { steps ->
                            steps.size == 1 && steps.all { it.state == OrderCompensationStepState.SUCCEEDED }
                        } == true
            }
