...
class OrderTest {
    ...
    @Test
    fun `benefit only order starts the same store acceptance clock`() {
        val fixture = menuFixture()
        val quote =
            MenuQuoteCalculator()
                .quote(
                    ...
                    lines = listOf(QuoteOrderLine(fixture.menu.id, listOf(fixture.option.id), 1)),
                ).single()
        val pricing =
            OrderPricingCalculator().calculate(
                lines = listOf(PricingLine(0, quote.menuId, Krw.of(quote.unitPriceKrw), 1, false)),
                couponDiscount = Krw.ZERO,
                pointsToUse = Krw.of(quote.unitPriceKrw),
            )
        val createdAt = Instant.parse("2026-07-30T00:00:00Z")
        ...
        assertThat(order.state).isEqualTo(io.github.kdh949.beanflow.ordering.internal.domain.OrderState.PAID)
        assertThat(order.paidAt).isEqualTo(createdAt)
        assertThat(order.acceptanceWarningAt).isEqualTo(createdAt.plusSeconds(120))
        assertThat(order.acceptanceDeadlineAt).isEqualTo(createdAt.plusSeconds(180))
    }
