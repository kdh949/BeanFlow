...
internal class MenuCatalogEndpointIntegrationTest(
    ...
    fun cleanDatabase() {
        jdbc.execute(
            ...
            TRUNCATE TABLE spring_session_attributes, spring_session, identity_login_attempt,
                ...
                merchant_menu_option, merchant_menu,
    ...
    fun `OWNER creates replaces no-ops replays and archives a complete Menu aggregate atomically`() {
        ...
        val created =
            mutate(
                ...
                content(menuId, optionId, configurationId, "카페 라테", 4_500),
    ...
    fun `STAFF is allowed while stale revoked cross-store and changed replay are explicit failures`() {
        ...
        val optionId = UUID.randomUUID()
        val configurationId = UUID.randomUUID()
        val payload = content(menuId, optionId, configurationId, "필터 커피", 4_000)
        ...
        mutate(
            ...
            actor,
            "menu-stale-key-0001",
            content(menuId, optionId, configurationId, "필터 커피", 4_000, expectedVersion = 7),
        ).andExpect(status().isConflict)
    ...
    fun `invalid aggregate references and identifier collisions do not leave partial rows`() {
        ...
        val invalid =
            content(menuId, optionId, configurationId, "콜드브루", 4_800)
    ...
    @Test
    fun `aggregate boundaries accept 100 options and 500 configurations then reject overflow`() {
        val storeId = seedStore()
        ...
        mutate(
            ...
            boundaryContent(storeId, UUID.randomUUID(), optionCount = 100, configurationCount = 500),
        ).andExpect(status().isCreated)
            .andExpect(jsonPath("$.options.length()").value(100))
            .andExpect(jsonPath("$.configurations.length()").value(500))

        listOf(
            boundaryContent(storeId, UUID.randomUUID(), optionCount = 101, configurationCount = 0),
            boundaryContent(storeId, UUID.randomUUID(), optionCount = 9, configurationCount = 501),
    ...
    fun `search index failure rolls back owner command and audit writes`() {
        ...
        try {
            mutate(
                ...
                content(menuId, UUID.randomUUID(), UUID.randomUUID(), "롤백 라테", 4_500),
    ...
    fun `concurrent identical Menu commands serialize to one write and one exact replay`() {
        ...
        val menuId = UUID.randomUUID()
        val payload = content(menuId, UUID.randomUUID(), UUID.randomUUID(), "동시 라테", 4_500)
            ...
            val statuses =
                listOf(
                    Triple(
                        firstStoreId,
                        actor,
                        content(firstMenuId, UUID.randomUUID(), UUID.randomUUID(), "시청 라테", 4_500),
                    ),
                    Triple(
                        secondStoreId,
                        secondSession,
                        content(secondMenuId, UUID.randomUUID(), UUID.randomUUID(), "강남 라테", 5_000),
                    ),
    ...
    fun `Store boundaries accept 1000 active Menus and 5000 active Options then reject overflow`() {
        ...
        mutate(
            ...
            boundaryContent(menuBoundStore, UUID.randomUUID(), optionCount = 0, configurationCount = 0),
        ).andExpect(status().isCreated)
        mutate(
            post("/api/v1/stores/$menuBoundStore/menus"),
            menuBoundActor,
        ...
        mutate(
            ...
            boundaryContent(optionBoundStore, UUID.randomUUID(), optionCount = 100, configurationCount = 0),
        ).andExpect(status().isCreated)
        mutate(
            post("/api/v1/stores/$optionBoundStore/menus"),
            optionBoundActor,
            "store-option-overflow-001",
            boundaryContent(optionBoundStore, UUID.randomUUID(), optionCount = 1, configurationCount = 0),
        ).andExpect(status().isBadRequest)
            .andExpect(jsonPath("$.code").value("INVALID_REQUEST"))
