...
@SpringBootTest
internal class PickupReservationRepositoryTest @Autowired constructor(
	...
	fun `same pickup source is idempotent and increments capacity once`() {
		...
		val command = ReservePickupCommand(
			orderId = orderId,
			storeId = storeId,
			pickupSlotId = slotId,
			expiresAt = Instant.parse("2026-07-28T00:05:00Z"),
	...
	private fun insertSlot(id: UUID, storeId: UUID, capacity: Long) {
		transactions.executeWithoutResult {
