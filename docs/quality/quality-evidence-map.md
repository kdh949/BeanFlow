...
| 고객 정밀 좌표가 응답·error body·log·metric·Audit에 남음 | raw-string parameter binding, 값 없는 검증 메시지, closed metric 어휘, Discovery의 durable write 부재 | `NearbyStoreDiscoveryIntegrationTest`의 응답/400 body/metric tag/`operations_audit_record` scan과 `NearbyStoreDiscoveryValidationTest`의 message 검사 | BR-28, ADR-020, `ApiExceptionHandler` echo 경로 |
| 검색 가능한 매장 정보가 placeholder로 활성화되거나 일부 매장이 조용히 제외됨 | 별도 `StoreDiscoveryProfile`, V33 migration coverage gate, startup precheck | empty/exact coverage 통과와 unresolved row migration 중단, missing/orphan/blank-name/non-point/extension 제거 startup 실패 | ADR-020, [V33](../../src/main/resources/db/migration/V33__create_merchant_store_discovery_profile.sql), nearby runbook |
