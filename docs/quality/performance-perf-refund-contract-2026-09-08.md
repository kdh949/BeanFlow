...
![성능 드라이버의 Toss 환불 멱등 계약 전](assets/perf-refund-contract/refund-before.png)

수정 후 (실제 패널의 시각과 축을 함께 확인):

![성능 드라이버의 Toss 환불 멱등 계약 후](assets/perf-refund-contract/refund-after.png)


환불 패널은 전후 각각 10분 구간의 전체 앱 시도율이다. 실행 ID는 annotation을 선택하며
환불 metric은 test ID label이 없어 해당 코호트만 필터링한 수치가 아니다. 수정 전 20/s와 수정 후
예열/5/s의 시도율 높낮이는 비교하지 않는다. unknown/lookup → request/succeeded의 결과 종류와
DB 코호트 검증을 함께 사용한다.
