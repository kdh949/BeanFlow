...
넣고, 고객 → 일회성 결제 → 매장 → 포인트 적립 → 부분/전액 환불 흐름을 실제 HTTP로 확인하는
방법과 실패 진단을 다룬다.
...
fixture가 유지된다. `smoke.sh`는 runtime OpenAPI에 선언된 operation만 호출하며 준비·callback
exact replay·위변조 거부·상태 조회, 매장 완료, 부분/전액 remaining 환불과 두 번째 결제의
`UNKNOWN → APPROVED` query 복구를 확인한다. 첫 주문은 10,000 KRW이고 완료 뒤 해당 order의
`ACCRUAL` 원장 source와 100 KRW balance delta를 deadline 안에 검증한다.

고객 UI는 `http://127.0.0.1:4173/app`, 매장 콘솔은 `/store`, 운영 콘솔은 `/ops`다. local demo도
인가를 우회하지 않으므로 `.demo-runtime/demo-identity.env`의 `CUSTOMER_TOKEN`, `STORE_OWNER_TOKEN`,
`PLATFORM_OPERATOR_TOKEN` 값을 각 화면의 인증 연결 폼으로 입력한다. JWT를 source, log나 문서에
복사하지 않는다.
