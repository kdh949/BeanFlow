...
7. [dashboard provider](../../infra/observability/central/grafana/provisioning/dashboards/beanflow.yml)와
   [Performance RCA dashboard](../../infra/observability/central/grafana/dashboards/beanflow-performance-rca.json),
   [Live Operations dashboard](../../infra/observability/central/grafana/dashboards/beanflow-live-operations.json)을
   각각 provisioning/dashboards와 `/var/lib/grafana/dashboards/beanflow`에 배치한다.
   평상시 현재 상태 확인은 [Live Operations 사용 안내](live-operations-dashboard.md)를 따른다.
...
아래는 해당 배포 시점의 기록이다. 이후 이미지·환불 driver 변경과 검증 결과는
[2026-09-08 잠금 구간 재측정](../quality/performance-lock-critical-section-retest-2026-09-08.md)을 참조한다.
실제 복구 전에 대상 서버의 현재 revision과 배포 입력을 확인한다.
