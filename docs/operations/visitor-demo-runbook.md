...
- V88은 빈 체험 테이블과 nullable 계정 컬럼을 추가한다. 기존 계정과 주문을 변환하지 않는다. V87은 [PR #189](https://github.com/kdh949/BeanFlow/pull/189)가 소유한다. 이 변경은 Draft 상태에서 검토하며, #189 병합 후 최신 main에서 마이그레이션 순서를 다시 검증한 뒤 V88을 병합·배포한다. V88을 먼저 적용하거나 out-of-order 설정으로 순서를 우회하지 않는다.
...
1. `/demo`에서 주문 처리 체험 시작. 새로운 고객·점주·매장·메뉴·픽업 슬롯·포인트·주문을 하나의 DB transaction으로 발급한다.
...
전체 여정, 재시도, 종료, 만료는 추가 play 테스트로 제공한다. 1440px 점주 화면과 390px 고객 화면을 점검한다. 모바일 고객 화면은 내용 전체를 스크롤할 수 있고 안내 접기로 주문 내용을 먼저 확인할 수 있다.
