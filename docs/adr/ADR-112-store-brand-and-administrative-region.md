...
### 리 단위 지역 어휘 Amendment (2026-08-15)

원 Decision은 지역 어휘를 시도·시군구·읍면동 3계층으로 정했다. Milestone 1에서 실제 법정동
자료를 적재한 결과 **폐지되지 않은 20,560행 중 15,209행(74%)이 리 단위**였고, 이 행들은 상위
읍면동 이름만 검색 가능한 채로 남았다. `강원특별자치도 춘천시 동면 감정리`에 있는 매장은
`동면`으로는 찾을 수 있어도 `감정리`로는 찾을 수 없었다.

리 단위를 검색 가능하게 만들기 위해 다음을 개정한다.

#### R1. `merchant_region`에 `ri` 열을 추가한다
...
`sigungu`·`eupmyeondong`과 같은 이유로 `NOT NULL DEFAULT ''`다. 리가 없는 행정구역이 다수이므로
`NULL`을 쓰지 않는다. 리 행의 `eupmyeondong`은 **여전히 상위 읍·면 이름을 담는다.** 리 이름이
`eupmyeondong`을 덮어쓰지 않으므로 리에 있는 매장은 읍·면 이름과 리 이름 **양쪽으로** 검색된다.

법정동 코드 뒤 2자리가 리 코드이므로 계층 판별에 새 자료가 필요하지 않다. 시드 원본도 그대로다.

#### R2. `REGION_RI` term 종류를 추가한다

[ADR-103](ADR-103-store-search-strategy.md) A1의 검색 term이 여섯에서 **일곱 종류**가 된다.
가중치는 다른 `REGION_*`과 같은 `0.80`이다. 리는 읍면동보다 좁지만 더 정확한 매칭이라고 볼 근거가
없고, 가중치 서열을 늘리면 BR-47이 정한 순서 관계만 복잡해진다.

리 행에 지정된 매장은 `REGION_*` term을 3행이 아니라 **4행** 갖는다. 리가 없는 지역은 종전대로
3행이며 빈 문자열로는 term을 만들지 않는다.

#### R3. 리 이름의 전국 중복은 반경 필터에 맡긴다

`상리`, `하리`, `신촌리` 같은 이름은 전국에 여러 개 있다. 지역명 term은 좌표·반경 필터와 함께
평가되므로 먼 곳의 동명 리가 결과에 섞이지 않는다. 리 이름만으로 전국을 조회하는 경로는 애초에
없다. 따라서 이름 중복을 해소하기 위한 별도 식별자나 상위 지역 강제 입력을 두지 않는다.

#### Amendment의 결과

- 지역 검색 대상이 5,351행에서 20,560행으로 넓어진다. 색인 term 총량은 리에 지정된 매장 수만큼만
  늘어나므로(매장당 1행) 색인 크기 영향은 매장 수에 비례하지 실제 시드 크기에 비례하지 않는다.
- `merchant_region` 시드 SQL이 열 하나만큼 커진다. 행 수는 변하지 않는다.
- 원 Decision의 Revisit Condition "법정동보다 세밀한 상권 단위(`홍대`, `가로수길`)"는 **그대로
  남는다.** 리는 법정동 체계 **안**의 단위라 이 Amendment로 해결되지만, 법정동 체계에 없는 상권
  이름은 여전히 별도 자료가 필요하다.
...
### 7. 리 이름을 `eupmyeondong` 열에 덮어쓰기 (리 Amendment 대안)

- 장점: 새 열도 새 term 종류도 없이 리 검색이 된다. 마이그레이션이 시드 재생성뿐이다.
- 단점: 리에 있는 매장이 상위 읍·면 이름으로는 검색되지 않는다. `동면`으로 찾던 것이 `감정리`로만
  찾히게 되어 **검색 가능 범위가 늘지 않고 이동할 뿐**이다. 열 이름과 내용도 어긋나 이후 지역
  집계·필터에서 읍면동 단위를 신뢰할 수 없다.

### 8. 리 행을 시드에서 제외 (리 Amendment 대안)

- 장점: 시드가 5,351행으로 줄어 파일 크기와 생성 시간이 4분의 1이 된다.
- 단점: 리에 주소를 둔 매장이 **지정할 지역 코드 자체를 갖지 못한다.** `region_code`를 `NOT NULL`로
  승격하는 순간 그 매장들 때문에 커버리지 마이그레이션이 실패해 배포가 막힌다. 지역 어휘를
  폐쇄 어휘로 둔 이상 실재하는 주소를 표현할 수 없는 어휘는 선택지가 아니다.
...
- 리 단위 지역이 `ri` 열에 담기고 같은 행의 `eupmyeondong`이 상위 읍·면 이름을 그대로 유지하는지
  검증한다. 리에 지정된 매장이 **읍·면 이름과 리 이름 양쪽으로** 검색되는지 확인한다.
- 리가 없는 지역의 `ri`가 빈 문자열이고 `REGION_RI` term이 만들어지지 않는지 검증한다.
- 전국에 중복되는 리 이름(`상리` 등)이 반경 밖에서는 결과에 섞이지 않는지 검증한다.
...
- **보강:** 시도 행을 `<시도>00000000`으로 가정할 수 없다. 세종특별자치시에는 그 코드가 없고
  ...
  코드 자릿수로 해야 한다.
- **후속 개정:** 시드 20,560행 중 15,209행이 리 단위였고, 이 행들은 리 이름으로 검색되지 않는
