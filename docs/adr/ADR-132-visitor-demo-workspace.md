...
기본 샘플은 전용 매장 발행 포인트 4,500원을 사용해 기존 BENEFIT_ONLY 승인으로 PAID를 만든다. 기본 주문과 재시작은 새 주문을 생성하며 기존 주문 전이를 되돌리지 않는다. 직접 주문은 기존 Toss 테스트 결제 흐름이다. fake PG나 성공 응답으로 교체하지 않는다.
...
## Consequences
인증 scope 검사와 발급 migration이 추가된다. 만료 후에도 금융 원장을 보존하므로 전용 환경의 보존/폐기 운영이 필요하다. 주문의 실제 PK/reference/픽업 번호는 동적으로 표시한다.

## Verification
격리/만료/종료/중복/동시성/rollback/profile 보호 테스트, Storybook interaction/a11y, 실제 route 검증. 실행 증거는 ExecPlan에 기록한다.
