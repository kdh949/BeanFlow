...
아래는 구현 시작 시점의 기준선이다. 현재 배포 revision은 `686ba078`이며 배포 후 결과와
잔여 문제는 Outcomes & Retrospective 및 재측정 보고서에 기록한다.
...
override와 driver 상태를 백업하고 API와 perf driver만 재생성한다. health/config 검증 실패 시
직전 image·driver 및 교체 전 합성 결제 사실을 복구한다.
...
1. ADR/계획 기록 및 기준선 조건과 background 오염 평가, 원인 증거 확보.
2. 성능 경로 regression을 먼저 실패시킨 뒤 최소 구현과 관련 회귀 검증.
3. 범위 제한 commit/push 및 exact SHA build workflow 성공 확인.
4. Doppler를 유지하는 API/driver 배포, runtime revision/health 및 다른 dependency identity 검증.
...
- [x] 실제 기준선 확보 및 환불 background로 격리 조건 불충족을 명시.
...
- [x] `686ba078` 전체 CI `34151795027` 및 이미지 workflow `34151796891` 성공.
- [x] 앱 서버 immutable image pull/revision 확인 및 8-file 배포 bundle checksum 검증.
- [x] 배포 및 Grafana 전후 검증.
- [x] PR134~140의 최신 head와 terminal CI SUCCESS, 각 현재 base의 MERGEABLE/CLEAN 확인.
...
계속되는 조회 부하를 확인했다. 배포 후보 `686ba078`의 전체 CI 및 API packaged portfolio/perf startup 검증이 통과했다.
이미지는 `ghcr.io/kdh949/beanflow-api@sha256:46f2167d5885596cf358bd1e962e6d22edc13660415ffedd5e1f8d7cffb2f576`이며
서버 API/driver 배포 및 재측정을 완료했다. 신규3788건의 환불·보상과 기존 이력 포함12058건 조회를 검증했다.
카운터 발급 평균은 5.142→0.388ms로 감소했지만 20/s 두 실행은 dropped102/152로 실패해 HTTP 개선은 입증되지 않았다.
금융 publication7576건은 미완료로 남았다. 2026-09-08 05:35 KST 이후 최종 조회에서
분리 PR134~140의 최신 head CI가 모두 SUCCESS이며 각 현재 base에서 MERGEABLE/CLEAN임을 확인했다.
PR140은 기존 비동기 테스트 1건의 첫 실패 후 같은 SHA의 실패 shard를 한 번 재실행해 통과했다.
실패 로그, REQUIRES_NEW 내부 commit과 바깥 transaction 사이의 테스트 대기 공백, 같은 head의
로컬 해당 클래스13개 통과를 PR에 기록했다. 부하 관련 성능 향상 증거로 사용하지 않는다.
당시에는 PR134가 PR136/138/139의 선행이며 PR140은 PR138 뒤에 병합하는 순서였다.
PR135/137은 main에 독립 병합하는 계획으로 남겼다. 2026-09-08 후속 확인에서 PR134~140은
모두 병합됐으며 main `8038e71`의 CI, CodeQL과 이미지 build가 통과했다.
이 확인은 새 서버 배포·부하 재측정의 증거가 아니다. 상세 결과와 남은 금융 publication은
[통합 재측정 보고서](../../quality/performance-lock-critical-section-retest-2026-09-08.md)에 기록한다.

## Revision Notes

- 2026-09-08: 실제 runtime과 baseline 진단을 반영한 최초 계획.
- 2026-09-08: 환불 driver 계약 오류와 미완료 환불 background 발견을 반영해 검증 범위 확대.
- 2026-09-08: 배포·부하 재측정·Grafana 캡처·분리 PR의 최종 원격 검증을 완료하고 계획 종료.
- 2026-09-08: 후속 main 병합 및 CI 결과를 검증 시점별로 구분해 추가했다.
