...
- 현재 core journey의 알려진 기능 공백은 customer coupon wallet **selection UI**다. 이미 발급된 쿠폰의
  actor-scoped backend/API read는 [Goal Stage 05 ExecPlan](core-journey-05-customer-coupon-wallet.md)에 따라
  구현됐고, plan status는 `ACTIVE`, backend/API slice의 `Implementation-Ready=true`,
  `Writes-Migration=false`다. Storybook MCP가 unavailable인 동안 UI slice는 blocked다. 현재 slice는 store
  scope만 판단하고 brand scope는 defer하며, Campaign 관리·발급·history와 wallet balance는 범위 밖이다.
