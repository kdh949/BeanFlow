...
- `OrderCompletedV2` cutover inventory, immutable completion snapshot과 SettlementItem 생성의 source unique
...
- `OrderCompletedV2` consumer transaction은 immutable snapshot을 검증하고 `(storeId,
  settlementDate)`의 `OPEN` Batch를 insert-or-read한 뒤 SettlementItem, Audit과
  `SettlementItemCreatedV1` publication을 함께 commit한다.
