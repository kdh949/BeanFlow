...
- event catalog는 `OrderCompletedV1`, `PaymentRefunded`, `PointsAccrued`, `PointsRestored`,
  `PointsAdjusted`, `SettlementItemCreated`, `SettlementAdjustmentCreated`를 Analytics input으로
  지정하지만 module, migration, listener, freshness projection, ReprocessingCase integration은 없다.
- Plan 10은 Refund allocation/`RECOVERY`와 Loyalty event 기반을, Plan 20은 Settlement Item event
  기반을 만든다. 이 계획은 해당 work의 actual event/version contract가 완료된 뒤 시작한다.
...
- producer transaction은 original financial fact와 Spring Modulith persistent publication을 함께 commit한다.
  Analytics listener는 producer repository의 latest state를 추측하지 않고 immutable payload를 소비한다.
...
- producer plan은 immutable source, occurrence/result time, integer cash/adjustment amount, needed completion-date
  reference를 event payload에 제공해야 한다. 기존 V1 required field 의미를 바꾸지 않고 부족한 값은 compatible
  optional field 또는 V2/approved ADR path로만 추가한다.
...
1. Plan 10/20 및 existing Ordering/Payment/Loyalty event payload, publication durability, source/version uniqueness를 audit한다.
...
- non-success Refund, customer-cancellation exclusion Audit, Settlement adjustment, PointsAccrued/Restored/Adjusted effects
...
- BR-31~32, ADR-023 and event-publication failure implementation evidence
...
- 2026-08-01: event catalog에는 Analytics consumer가 있지만 immutable delta payload, receipt/freshness owner,
  backfill checkpoint 구현은 없다. 이 계획은 producer source of truth를 복제하지 않고 contract gate를 둔다.
...
미구현 상태다. Plan 10/20과 producer event contract의 actual validation이 완료된 뒤 listener를
활성화한다. 완료 시 fixture tie-out, seven-day/backfill recovery, freshness failure, measurement 결과를
actual value로 기록한다.
