...
```text
고객
  가입·로그인 → 매장 탐색 → 매장·메뉴·픽업 확인 → 쿠폰 지갑 → 서버 견적 확인 → 주문 생성
       → 결제 시도·확인·UNKNOWN 복구 → 내 주문/추적·알림함 → 취소 또는 재주문
...
쿠폰 지갑과 매장 범위 선택 UI는 실제 route와 live Storybook 검증까지 연결됐다.
...
| 매장·메뉴·픽업 확인 | 운영시간·주문 가능 여부·메뉴·실제 가장 이른 픽업을 분리해 확인 | public store/catalog read | `GET /stores/{storeId}`, `/stores/{storeId}/menus`, `/stores/{storeId}/pickup-slots` | `/app/stores/:storeId`, `/app/cart` | `scripts/demo/seed.sh`, Store/Cart stories | `StoreCatalog*`, `StoreSearch*`, `Ordering.test.tsx`, live Storybook | customer-merchant screen contract plan, ADR-076/117 | Integrated | deployed customer journey는 Not run |
| 보유 쿠폰 지갑·선택 | 이미 발급된 쿠폰 중 매장에 적용 가능한 것을 확인 | Customer-scoped Promotion DTO projection | `GET /me/coupons?storeId=&cursor=&limit=` | `/app/coupons` | `CustomerCouponWalletIntegrationTest`, Coupon Wallet stories | integration/OpenAPI/auth regression, `CustomerBenefits.test.tsx`, live Storybook | Stage 05 core-journey slice | Integrated | deployed customer journey는 Not run |
...
| 주문 견적·생성 | 비예약 서버 견적을 확인하고 같은 fingerprint일 때만 주문화 | quote, final locked revalidation, reservation, coupon/point calculation | `POST /me/order-quotes`, `POST /orders` | `/app/cart`, `/app/checkout/:orderId` | Cart stories | quote/order concurrency·idempotency tests, `Ordering.test.tsx`, live Storybook | customer-merchant screen contract plan, ADR-116 | Integrated | Provider sandbox와 deployed customer journey는 Not run |
...
| 내 주문·추적·취소·재주문 | 실제 lifecycle과 immutable 거래 요약을 보고 후속 행동 수행 | customer order projection, cancellation/reorder facade | `GET /me/orders`, `/me/orders/{orderReference}`, `POST /me/orders/{orderReference}/cancellations`, `/reorders` | `/app/orders`, `/app/orders/:orderReference` | Order Detail stories | customer order/lifecycle/cancellation/reorder tests, `OrderPages.test.tsx`, live Storybook | customer-merchant screen contract plan, ADR-099 | Integrated | deployed journey E2E는 Not run |
| 고객 알림함·수신 설정 | 읽지 않은 거래 알림을 확인하고 마케팅 수신 여부를 관리 | InboxItem, preference, unread summary | `GET /me/notification-summary`, `GET/PATCH /me/notifications`, `GET/PUT /me/notification-preferences` | `/app/notifications`, customer shell bell | Notification stories | Notification integration/migration/API tests, frontend unit, live Storybook | customer-merchant screen contract plan, ADR-104 | Integrated | 실제 delivery Provider sandbox는 Not run |
...
| 접근 매장 선택·주문보드 | 실제 단계 진입 시각과 경과 시간을 보며 주문 수락·제조·완료 | membership scoped store list, ETag board projection and transition | `GET /merchant/me/stores`, `GET /stores/{storeId}/orders`, `POST /stores/{storeId}/orders/{orderReference}/transitions` | `/store` | fixed-clock Board stories | board migration/query/ETag/transition tests, frontend unit, live Storybook | customer-merchant screen contract plan, ADR-100 | Integrated | combined customer→merchant deployed E2E는 Not run |
| 부분 환불 | 서버가 계산한 안전한 주문 맥락과 line/quantity를 보고 환불 | preview, partial refund, point recovery states | `POST /stores/{storeId}/orders/{orderReference}/refund-previews`, `/refunds` | `/store/refunds/:storeId/:orderReference` | Refund stories | refund preview/execute/stale/unresolved/idempotency tests, frontend unit, live Storybook | customer-merchant screen contract plan, ADR-108 | Integrated | Provider sandbox와 deployed recovery는 Not run |
