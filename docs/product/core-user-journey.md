...
| 매장·메뉴 확인 | 운영시간·주문 가능 여부와 메뉴를 확인 | public store/catalog read | `GET /stores/{storeId}`, `/stores/{storeId}/menus` | `/app/stores/:storeId`, `/app/cart` | `scripts/demo/seed.sh`, Store/Cart stories | `StoreCatalog*`, `StoreSearch*`, `Ordering.test.tsx`, live Storybook | immediate checkout plan, ADR-133/135 | Integrated | deployed customer journey는 Not run |
...
| 주문 견적·생성 | 비예약 서버 견적을 확인하고 같은 fingerprint일 때만 즉시 결제 초안을 생성 | quote, final locked revalidation, immutable input snapshot | `POST /me/order-quotes`, `POST /orders` | `/app/cart`, `/app/checkout/:orderReference` | Cart stories | quote/order concurrency·idempotency tests, `Ordering.test.tsx`, live Storybook | immediate checkout plan, ADR-123/133/134 | Integrated | Provider sandbox와 deployed customer journey는 Not run |
