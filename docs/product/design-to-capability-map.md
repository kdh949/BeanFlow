...
| `4b 메뉴·가격` | 점주 | 메뉴·옵션·가격 관리 | Merchant | `GET /stores/{storeId}/menu-catalog`, `GET/POST /stores/{storeId}/menus`, `GET/PUT /stores/{storeId}/menus/{menuId}/trade-content`, `POST /stores/{storeId}/menus/{menuId}/archive`, 기존 display-content API | P1 | ACTIVE/ARCHIVED keyset 목록, Menu·Option·Configuration·requirement 전체 생성/교체·보관, 거래 version·멱등성·검색 원자 반영과 `/store/catalog` 소비 UI가 구현됐다(BR-52, ADR-118). 표시 metadata/image version은 거래 version과 분리된다. |
