...
| SP-14 | Accepted | Support Console은 기존 frontend의 격리 `/support` route를 사용하고 Operations와 Keycloak PKCE memory-only token을 공유한다. exact PII는 POST body, reveal은 route-local 메모리만 사용하며 navigation/expiry/Case terminal/logout/permission loss 때 제거한다. | ADR-090, ADR-092, BR-41 |
...
### SP-23 담당 상담원의 직접 처리 (2026-09-18)

**Accepted.** 신규 고객센터 업무는 기존 업무 권한·활성 상담 담당자·대상 연결을 근거로
본인확인과 고객·매장·관리자·운영자의 승인 없이 실행한다. SP-02/18/19/20/21/22의
인증·승인 선행조건을 신규 정책에서 대체한다. 등록→실행은 유지하고 같은 상담원이 수행한다.
요청/수정 유효기간은 15분이며 만료 시 인증 없이 재작성한다. 목록은 마스킹하고 정보 보기로
감사 후 원문을 연다. 일반 grant 10분/3회 또는 5분/1회, 긴급 grant 2분/1회, 화면 60초 제거를
유지한다. 긴급 사후 검토는 다른 업무의 선행조건이 아니다. 비용 SUPPORT_DECISION을 허용하며
비용 미확정·지급 한도·금융 결과 불명·중복 방지 규칙은 유지한다. 실제 인증/승인을 가장하지 않는다.
기존 미실행 요청은 409 재작성, 시작된 금융 처리와 완료 응답은 기존 복구 경계를 유지한다.
구체적인 계약과 검증은 [ADR-136](../adr/ADR-136-support-direct-processing.md)을 따른다.

#### SP-23 보완: 직접 처리 전환 경계와 보상 상한 위험 수용 (2026-09-18)

직접 처리 요청의 요청자·실행자·현재 상담 담당자는 같아야 한다. 상담 담당자를 바꾸면
새 담당자가 새 요청을 작성하며 기존 직접 처리 요청의 실행자만 변경하지 않는다.
이전 정책의 미실행 승인 요청은 Operations에서도 조회와 재작성 안내만 제공한다.
과거 완료 결정의 동일 키 응답과 금융 후속 복구는 유지한다. 종료된 본인확인 쓰기는
인증된 정상 경로 호출에서 본문·멱등키를 검증하지 않고 410을 반환한다.

보상의 `supportedAmountMaximumKrw` 초과에 대한 평가·등록 단계의 별도 거부는 추가하지 않는다.
내부 담당자가 금액 정책을 준수한다는 운영 전제로 현재 직접 처리 평가를 유지한다.
현재 누적 실행 한도는 그대로 강제한다. 상한 초과 요청의 등록 후 실행 실패 가능성과,
향후 누적 한도가 완화되면 지원 금액 기준을 넘긴 지급이 가능해지는 위험을 수용한다.
누적 한도 변경 또는 내부 담당자 외 사용 확대 시 이 결정을 재검토한다.
