...
| BR-03 | Order와 OrderLine option snapshot, 세 자원 Reservation | 주문 생성, terminal source의 빠른 재주문, PointLot 예약 시점 보장, `UNKNOWN`이어도 effective lease 만료; late approval은 void/refund | 현재 조건 전체 재검증, 한 로컬 tx, active source unique, guarded expiry/approval, 부분 재주문 금지 | ADR-004, ADR-005, ADR-011, ADR-013, ADR-077 | Implemented |
| BR-04 | Menu, MenuConfiguration | 점주 품절·판매 재개, 최종 주문 판매 상태 검증 | normalized option set, Store shared/exclusive lock | ADR-026, ADR-118 | Ready |
| BR-05 | PickupSlot, PickupReservation | reserve → confirm/release | capacity guard, active order unique | ADR-005 | Ready |
| BR-06 | Order, Refund, 세 자원 Reservation, NotificationDelivery, BenefitRestorationPolicy, OrderCompensationCase, AcceptanceTimeoutWork | 수락 경고·timeout·거절 보상·만료 혜택 disposition | guarded Order transition, trigger×benefit policy snapshot, idempotent compensation, case trigger, durable timeout wakeup | ADR-015, ADR-028, ADR-033, ADR-041, ADR-058, ADR-059 | Clean-cutover gate passed; trigger-aware compensation foundation pending |
...
| BR-11 | Payment(type=BENEFIT_ONLY), Order와 세 자원 Reservation, OrderCompensationCase | 주문 생성 tx 안의 PG 없는 승인·예약 확정, 고객 취소 시 Refund 없는 PAYMENT NOT_REQUIRED | order/payment source unique, create/cancel tx 원자성, 공통 다섯 step | ADR-016, ADR-039 | Ready |
...
| BR-25 | IdempotencyRecord, StoreCommandIdempotency, CancellationCommandIdempotency, PointAdjustment command, RepairProposal | 주문 생성·빠른 재주문 최초 response 재생, Payment UNKNOWN 현재 상태, 고객 취소·매장 전이·point adjustment terminal response 재생, replay 표시 부재, 2인 복구 명령 | actor/operation/key unique + canonical payload hash, target ID 포함, operation 승격, 기존 직렬화 root·외부 결과 불명 기반 모델 선택, proposal guarded transition | ADR-007, ADR-025, ADR-032, ADR-053, ADR-057, ADR-064, ADR-066, ADR-077 | Implemented |
