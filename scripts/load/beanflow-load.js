...
export function quoteOrder() {
  ...
  createQuotedOrder(customer, scenario);
}
...
function requestParams(actor, requestScenario, route, idempotency) {
  ...
  return { headers, tags: { name: route, route: route.slice(route.indexOf(' ') + 1) }, timeout: '20s' };
}
