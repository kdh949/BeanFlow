...
def build_regions(active: dict[str, str]) -> list[tuple[str, str, str, str, str]]:
    ...
    for code in sorted(active):
        ...
        # 리 단위 행은 자기 이름 대신 상위 읍면동 이름을 읍면동 열에 담는다.
        # 스키마에 리 열이 없고 검색 term도 시도/시군구/읍면동 세 종류뿐이다(ADR-112).
        # 리는 선택 가능한 코드로만 남고 리 이름 자체로는 검색되지 않는다.
        ...
        regions.append((code, sido, sigungu, eupmyeondong, full_name))
    return regions
...
def render_sql(regions: list[tuple[str, str, str, str, str]]) -> str:
    out = io.StringIO()
    out.write(f"-- Generated by scripts/generate-region-seed.py. Do not edit by hand.\n")
    out.write(f"-- Source: {SOURCE_URL} ({SOURCE_FORM_DATA['codeseId']})\n")
    out.write(f"-- Source SHA-256: {EXPECTED_SOURCE_SHA256}\n")
    out.write(f"-- Active rows: {len(regions)}\n")
    out.write("--\n")
    out.write("-- 폐지되지 않은 법정동만 담는다. 시도/시군구/읍면동은 법정동 코드 계층으로\n")
    out.write("-- 상위 행 이름을 잘라 낸 값이고 full_name은 원본 법정동명 그대로다.\n")
    out.write("-- ON CONFLICT DO NOTHING이라 재실행해도 행 수가 변하지 않는다.\n\n")
    out.write(f"INSERT INTO {TABLE} (code, sido, sigungu, eupmyeondong, full_name) VALUES\n")
    rendered = [
        f"    ({quote(code)}, {quote(sido)}, {quote(sigungu)}, {quote(eupmyeondong)}, {quote(full_name)})"
        for code, sido, sigungu, eupmyeondong, full_name in regions
    ]
    out.write(",\n".join(rendered))
