...
import { Button, ButtonLink, FeedbackState, PageHeading, QuantityStepper, SelectField, TextAreaField, TextField } from "../../design-system";
...
function RefreshOrderCard({ item, storeId, now, busy, rejecting, rejectionReason, onBeginReject, onCancelReject, onReasonChange, onAction }: { item: StoreOrderBoardItem; storeId: string; now: Date; busy: boolean; rejecting: boolean; rejectionReason: string; onBeginReject: () => void; onCancelReject: () => void; onReasonChange: (value: string) => void; onAction: (action: StoreOrderAction, reason?: string, preparationMinutes?: number) => void }) {
  ...
  const preparationValid = Number.isSafeInteger(preparation) && preparation >= 1 && preparation <= 120;
  ...
  return <article className={`bfr-order-card ${item.acceptancePhase === "WARNING" ? "is-warning" : ""}`} aria-label={`주문 ${item.pickupNumber}`}>
    ...
    <dl><div><dt><CalendarDays size={13} />영업일</dt><dd>{item.pickupBusinessDate}</dd></div><div><dt><Clock3 size={13} />{item.pickupWindowStart ? "픽업 시간" : item.lifecycle?.estimatedReadyAt ? "준비 예상" : "접수 마감"}</dt><dd>{shortDateTime.format(new Date(item.pickupWindowStart ?? item.lifecycle?.estimatedReadyAt ?? item.acceptanceDeadlineAt!))}</dd></div>{elapsed ? <div><dt><Clock3 size={13} />현재 단계</dt><dd>{elapsed}</dd></div> : null}</dl>
    ...
    {item.allowedActions.includes("ACCEPT") ? <section aria-label={`${item.pickupNumber} 준비시간`}><TextField label="준비시간 (분)" type="number" value={preparationMinutes} error={preparationMinutes && !preparationValid ? "1분부터 120분까지 입력해 주세요." : undefined} onValueChange={setPreparationMinutes} /><div className="bfr-card-actions">{[5, 10, 15, 20, 30].map((minutes) => <Button key={minutes} size="sm" variant={preparationMinutes === String(minutes) ? "brand" : "secondary"} onClick={() => setPreparationMinutes(String(minutes))}>{minutes}분</Button>)}</div></section> : null}
    <div className="bfr-card-actions">{item.allowedActions.map((action) => action === "REJECT" ? <Button key={action} size="sm" variant="danger" disabled={busy} onClick={onBeginReject}>{storeOrderActionLabels[action]}</Button> : <Button key={action} size="sm" variant="brand" loading={busy} disabled={action === "ACCEPT" && !preparationValid} onClick={() => onAction(action, undefined, action === "ACCEPT" ? preparation : undefined)}>{busy ? "처리 중" : storeOrderActionLabels[action]}</Button>)}<ButtonLink size="sm" variant="ghost" to={`/store/refunds/${storeId}/${item.orderReference}`}>부분 환불</ButtonLink></div>
