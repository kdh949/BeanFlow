...
export function OpsRefundPage({ initialPartial = false }: { initialPartial?: boolean }) {
  ...
  return <div className="console-page narrow-console-page">
    ...
    <form className="surface-card operation-form" onSubmit={(event) => void submit(event)}>
      ...
      <Button block disabled={!paymentId.trim() || !reason.trim() || (partial && !orderLineId.trim())} loading={loading} type="submit"><RotateCcw size={17} /> {loading ? "환불 요청 중" : partial ? "부분 환불 요청" : "전액 환불 요청"}</Button>
    </form>
...
export function RefundResult({ result }: { result: Refund }) {
  return <section className="surface-card result-card"><div><span className="eyebrow">REFUND {compactId(result.refundId)}</span><StatusBadge state={result.state} /></div><h2>{result.state === "SUCCEEDED" ? "현금 환불이 확인되었습니다" : "환불 결과를 확인 중입니다"}</h2><p>{result.state === "SUCCEEDED" ? `${won.format(result.cashRefundedKrw ?? result.cashRefundRequestedKrw)} 환불 완료` : "202 또는 미확정 상태는 성공이 아닙니다. 같은 키로 재요청하면 기존 작업을 조회합니다."}</p><dl className="detail-list"><div><dt>요청 금액</dt><dd>{won.format(result.cashRefundRequestedKrw)}</dd></div><div><dt>포인트 복구</dt><dd>{result.pointsRestorationState}</dd></div><div><dt>문의 코드</dt><dd><code>{result.correlationId}</code></dd></div></dl></section>;
}
