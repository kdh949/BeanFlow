...
export { StatusBadge } from "../design-system";

export function LoadingState({ label = "불러오는 중" }: { label?: string }) {
  return (
    <FeedbackState kind="loading" title={label} description="잠시만 기다려 주세요." />
  );
}
...
export function EmptyState({ title, description, action }: {
  ...
  return (
    <FeedbackState kind="empty" title={title} description={description} action={action} />
  );
}
...
export function ErrorState({ error, retry }: { error: unknown; retry?: () => void }) {
  ...
  return (
    <FeedbackState
      ...
      description={apiError?.message ?? "네트워크 연결을 확인하고 다시 시도해 주세요."}
