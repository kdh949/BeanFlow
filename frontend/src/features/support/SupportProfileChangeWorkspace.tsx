...
export function ProfileFields({ purpose, values, onChange, disabled }: { purpose: ProfilePurpose; values: ProfileValues; onChange: (values: ProfileValues) => void; disabled: boolean }) {
  ...
  return <div className="operation-form">
    {descriptor.risk === "R4" ? <InlineNotice title="인증정보 재등록 요청" description="비밀번호나 인증 토큰을 입력하거나 조회하지 않습니다. 실행하면 기존 소유 서비스에 재등록 의도를 전달합니다." /> : <p>입력값은 제출 후 또는 화면을 벗어나면 지워집니다. 요청을 실행할 때 같은 값을 다시 입력합니다.</p>}
...
/** Composes current profile metadata, typed direct changes, request execution and notification follow-up. */
export function SupportProfileChangeWorkspace({ supportCase, initialProfileChangeId, onBusyChange }: { supportCase?: Case; verification?: Verification | null; initialProfileChangeId?: string; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <section className="management-workspace" aria-label="정보 정정 업무"><h2>정보 정정</h2>
    ...
    {id ? <ProfileInspection key={id} id={id} supportCase={supportCase} onBusyChange={setActive} /> : supportCase ? <CreateProfileChange supportCase={supportCase} onBusyChange={setActive} onCreated={created => { setId(created); }} /> : <EmptyState title="상담 건에서 정정을 시작해 주세요" description="기존 요청 찾기에서 요청과 변경 상태를 확인할 수 있습니다." />}
  </section>;
}

function CreateProfileChange({ supportCase, onCreated, onBusyChange, revision }: { supportCase: Case; verification?: Verification | null; onCreated: (id: string) => void; onBusyChange: (busy: boolean) => void; revision?: Workflow }) {
  const targets = supportCase.subjectLinks.filter(link => ["CUSTOMER", "STORE", "DELIVERY"].includes(link.subjectType));
  const [linkId, setLinkId] = useState((revision ? targets.find(link => link.subjectId === revision.profileChange.subjectId && isSupportSubjectSelectable(link))?.linkId : targets.length === 1 ? targets.find(isSupportSubjectSelectable)?.linkId : "") ?? "");
  ...
  const descriptor = profilePurposes[purpose];
  const raw = useProfileValues(`${linkId}:${purpose}`);
  ...
  const busy = preparing || command.busy, frozen = busy || !!command.pending;
  ...
  const verified = !!link && isSupportSubjectSelectable(link);
  const valid = validProfileValues(purpose, raw.values) && !!reason.trim() && !!evidence.trim() && (descriptor.risk !== "R4" || resetAcknowledged);
  const eligible = link && current && verified && !["RESOLVED", "CLOSED"].includes(supportCase.state);
  async function submit() {
    if (busy || preparingRef.current || !current || !valid || (!command.pending && !eligible)) return;
    ...
    try {
      ...
      const common = { expectedProfileVersion: current.currentProfileVersion, reason: reason.trim(), evidenceDigest };
  ...
  return <div className="surface-card management-card management-workspace"><h3>{revision ? "정정안 수정" : "새 정보 정정"}</h3>
    ...
    {!targets.length ? <EmptyState title="정정할 대상이 없습니다" description="상담에 고객·매장·외부 배달원을 연결해 주세요." /> : context.state.status === "loading" ? <LoadingState label="현재 프로필 조건을 읽는 중" /> : context.state.status === "failed" ? <ErrorState error={context.state.error} retry={context.reload} /> : current ? <p>현재 버전 {current.currentProfileVersion} · 담당 상담원 직접 처리</p> : null}
    ...
    <Button disabled={busy || !valid || (!command.pending && !eligible)} onClick={() => void submit()}>{command.pending ? "같은 내용으로 정정 결과 확인" : revision ? "수정한 정정안 제출" : descriptor.risk === "R3" || descriptor.risk === "R4" ? "정정 요청 등록" : "확인한 정보 정정"}</Button>
  </div>;
}

function ProfileInspection({ id, supportCase, onBusyChange }: { id: string; supportCase?: Case; verification?: Verification | null; onBusyChange: (busy: boolean) => void }) {
  ...
  const [digest, setDigest] = useState(""), [message, setMessage] = useState(""), [assignee, setAssignee] = useState<OperatorSelection | null>(null), [assignmentReason, setAssignmentReason] = useState(""), [revising, setRevising] = useState(false), [revisionBusy, setRevisionBusy] = useState(false);
  ...
  useEffect(() => { onBusyChange(frozen); return () => onBusyChange(false); }, [frozen, onBusyChange]);
  const expired = useExpired(value?.executionExpiresAt);
  ...
  return <div className="surface-card management-card management-workspace"><h3>정정 검토와 실행</h3>
    {message ? <p role="status">{message}</p> : null}<SensitiveResult command={sensitive} />{command.failure ? <ErrorState error={command.failure} /> : null}{command.pending ? <InlineNotice tone="warning" title="후속 요청 결과를 확인하지 못했습니다" description="같은 요청으로 확인할 때까지 다른 명령을 잠급니다." action={<Button loading={command.busy} onClick={() => void command.retry()}>같은 후속 요청 확인</Button>} /> : null}
    {read.state.status === "loading" ? <LoadingState label="현재 정정과 실행 조건을 읽는 중" /> : read.state.status === "failed" ? <ErrorState error={read.state.error} retry={read.reload} /> : value && profile ? <>
      <p className="support-case-reference">정정 ID {id}</p>{profile.authorizationBasis === "LEGACY" ? <InlineNotice title="이전 정책의 요청입니다" description="실행하지 않은 변경은 같은 상담에서 새 요청으로 작성해 주세요. 기존 변경 이력과 알림 후속 처리는 유지됩니다." /> : null}<p>{profilePurposes[profile.purpose].label} · <StatusText state={profile.state} label={profile.state === "EXECUTED" ? "정보 변경 완료" : "정보 변경 대기"} /></p>
      <dl className="detail-list"><dt>대상 ID</dt><dd className="support-case-reference">{profile.subjectId}</dd><dt>요청 당시 / 현재 버전</dt><dd>{profile.expectedProfileVersion} / {value.currentProfileVersion}</dd><dt>요청 내용 해시</dt><dd className="support-case-reference">{profile.payloadDigest}</dd>{profile.maskedBefore !== null ? <><dt>변경 전</dt><dd>{profile.maskedBefore}</dd><dt>변경 후</dt><dd>{profile.maskedAfter}</dd></> : null}</dl>
      {approval ? <p>처리 단계 · <StatusText state={approval.state} label={approvalLabels[approval.state]} /></p> : null}
      ...
      <div className="button-row"><Button variant="secondary" disabled={frozen} onClick={() => { raw.clear(); read.reload(); }}>정정 상태 새로고침</Button><ButtonLink variant="secondary" to={`/support/profile-changes/${id}`}>정정 건 주소</ButtonLink></div>
      {(allowed("EXECUTE") || sensitive.pending) && !revising ? <><ProfileFields purpose={profile.purpose} values={raw.values} onChange={raw.setValues} disabled={busy || command.pending} /><p role="status">{matches ? "입력한 내용이 처리 요청과 일치합니다" : "검토할 값을 다시 입력하면 처리 요청과 대조합니다"}</p></> : null}
      {(allowed("EXECUTE") || sensitive.pending) && !revising ? <Button disabled={busy || command.pending || (!sensitive.pending && !matches) || !validProfileValues(profile.purpose, raw.values)} onClick={() => void execute()}>{sensitive.pending ? "같은 내용으로 정정 결과 확인" : "변경 실행"}</Button> : null}
      ...
      {revising && supportCase ? <><CreateProfileChange supportCase={supportCase} revision={value} onBusyChange={setRevisionBusy} onCreated={() => { setRevising(false); read.reload(); }} /><Button variant="ghost" disabled={revisionBusy} onClick={() => setRevising(false)}>수정 닫기</Button></> : null}
