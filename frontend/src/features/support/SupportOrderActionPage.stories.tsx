...
export const PermissionRequired: Story = { play: async ({ canvas }) => { await expect(canvas.getByRole("heading", { name: "주문 변경 요청과 실행" })).toBeVisible(); await expect(await canvas.findByText("문의 코드 REQUEST-SCOPE")).toBeVisible(); await expect(canvas.queryByRole("button", { name: "확인한 주문 변경 실행" })).not.toBeInTheDocument(); } };
