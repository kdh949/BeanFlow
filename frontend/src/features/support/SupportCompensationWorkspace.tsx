...
const basisLabels: Record<NonNullable<Payload["evidenceBasis"]>, string> = { SUPPORT_DECISION: "상담원 결정", STORE_CONSENT: "매장 동의", OPERATIONS_FINDING: "운영 조사 결과", CONTRACTUAL_RULE: "계약 규칙" };
...
export function SupportCompensationWorkspace({ supportCase, initialCompensationId, initialIncidentId, onBusyChange }: { supportCase?: Case; verification?: Verification | null; initialCompensationId?: string; initialIncidentId?: string; onBusyChange?: (busy: boolean) => void }) {
...
function CreateCompensation({ supportCase, initialIncidentId, onCreated, onBusyChange }: { supportCase: Case; verification?: Verification | null; initialIncidentId?: string; onCreated: (id: string) => void; onBusyChange: (busy: boolean) => void }) {
  const subjects = supportCase.subjectLinks.filter(link => link.subjectType === "CUSTOMER" && link.linkId);
  ...
  const [orderId, setOrderId] = useState(orders.find(isSupportSubjectSelectable)?.subjectId ?? ""), [incident, setIncident] = useState<{ value: CompensationIncident; binding: string } | null>(null), [incidentBusy, setIncidentBusy] = useState(false);
  const incidentBinding = `${supportCase.caseId}|${subjectLinkId}|${orderId}`;
  ...
  const [basis, setBasis] = useState<NonNullable<Payload["evidenceBasis"]>>("SUPPORT_DECISION"), [costEvidence, setCostEvidence] = useState(""), [evidence, setEvidence] = useState("");
  ...
  useEffect(() => { sequence.current++; setEvaluation(null); }, [orderId, incidentId, benefit, amount, responsibility, share, basis, costEvidence, evidence, template, subjectLinkId]);
  const evaluationExpired = useExpired(evaluation?.result.expiresAt);
  ...
  const valid = (!orderId || !!selectedOrder) && !!incidentId && !selectedIncident?.benefitIssued && currentVersion !== undefined && amountKrw !== undefined && Number.isSafeInteger(amountKrw) && amountKrw > 0 && platformShare !== null && (!storeCost || !!costEvidence.trim()) && (benefit !== "COUPON" || (!!template && !!orderId));
  async function evaluate() {
    if (!verified || !active || !valid || busy || !incidentId || amountKrw === undefined || currentVersion === undefined || platformShare === null) return;
    const generation = ++sequence.current; setPreparing(true); setError(null); setEvaluation(null);
    try {
      const body: Payload = { incidentId: incidentId.trim(), orderId: orderId || null, expectedTargetVersion: currentVersion, benefitType: benefit, amountKrw, couponTemplateId: benefit === "COUPON" ? template!.templateId : null, responsibility, evidenceBasis: storeCost ? basis : null, costEvidenceDigest: storeCost ? await supportDigest(costEvidence.trim()) : null, platformShareBps: platformShare, storeShareBps: shares![1], subjectLinkId };
  ...
  return <div className="surface-card management-card management-workspace"><SelectField label="보상 대상" value={subjectLinkId} onValueChange={setSubjectLinkId} disabled={busy}><option value="">대상 선택</option>{subjects.map(link => <option key={link.linkId} value={link.linkId} disabled={!isSupportSubjectSelectable(link)}>{supportSubjectLabel(link)}</option>)}</SelectField>
    <h3>새 보상 요청</h3>
    {!active ? <InlineNotice tone="info" title="종결된 상담에서는 새 보상을 요청할 수 없습니다" description="필요하면 새 상담을 접수해 주세요." /> : !verified ? <InlineNotice tone="info" title="처리할 고객을 선택해 주세요" description="현재 상담에 연결된 고객을 선택하세요." /> : null}
...
function CompensationInspection({ id, caseId, onBusyChange }: { id: string; caseId?: string; onBusyChange: (busy: boolean) => void }) {
  ...
  const [reviewed, setReviewed] = useState(false), [assignee, setAssignee] = useState<OperatorSelection | null>(null), [assignmentReason, setAssignmentReason] = useState(""), [message, setMessage] = useState("");
  ...
  return <div className="surface-card management-card management-workspace">
    ...
    {read.state.status === "loading" ? <LoadingState label="현재 보상 조건과 권한을 읽는 중" /> : read.state.status === "failed" ? <ErrorState error={read.state.error} retry={read.reload} /> : value ? <>
      <p className="support-case-reference">보상 ID {id}</p>{value.request.authorizationBasis === "LEGACY" ? <InlineNotice title="이전 정책의 요청입니다" description="아직 지급하지 않은 요청은 같은 상담에서 새로 작성해 주세요. 지급된 혜택의 후속 처리는 계속할 수 있습니다." /> : null}<StatusText state={value.request.state} label={compensationStates[value.request.state]} />
      ...
      {value.approval ? <p>처리 단계 · <StatusText state={value.approval.state} label={approvalStates[value.approval.state]} /></p> : null}
      ...
      <div className="button-row"><Button variant="secondary" onClick={() => { setReviewed(false); read.reload(); }} disabled={busy}>보상 상태 새로고침</Button><ButtonLink variant="secondary" to={`/support/compensations/${id}`}>보상 건 주소</ButtonLink></div>
      ...
      {expired ? <InlineNotice tone="warning" title="처리 요청의 유효 시간이 지났습니다" description="같은 상담에서 새 요청을 작성해 주세요. 이미 지급한 혜택의 알림은 계속 처리할 수 있습니다." /> : null}
    </> : null}
  </div>;
}
