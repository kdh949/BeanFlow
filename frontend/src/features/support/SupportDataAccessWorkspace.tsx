...
type AccessCase = { caseId: string; state: string; subjectLinks: readonly (SupportSubjectDisplaySource & { linkId: string; subjectId: string; subjectType: string })[] };

/** One explicit action creates a field-scoped grant and reveals through the audited endpoint. */
export function SupportDataAccessWorkspace({ supportCase, initialGrantId, onBusyChange }: { supportCase?: AccessCase; session?: Session | null; initialGrantId?: string; onBusyChange?: (busy: boolean) => void }) {
  const targets = supportCase?.subjectLinks.filter(link => ["CUSTOMER", "STORE", "DELIVERY"].includes(link.subjectType)) ?? [];
  const [linkId, setLinkId] = useState(targets.length === 1 && isSupportSubjectSelectable(targets[0]!) ? targets[0]!.linkId : "");
  const target = targets.find(link => link.linkId === linkId && isSupportSubjectSelectable(link));
  const [purpose, setPurpose] = useState<Session["purpose"]>("CASE_RESOLUTION");
  ...
  const command = useSupportCommand(`data-access-request:${supportCase?.caseId ?? ""}`, () => {});
  ...
  const active = supportCase && !["RESOLVED", "CLOSED"].includes(supportCase.state);
  const available = target ? personalFieldsBySubject[target.subjectType as keyof typeof personalFieldsBySubject] : [];
  function requestGrant() {
    if (!active || !target || !fields.length || locked || !fields.every(field => available.includes(field))) return;
    const caseId = supportCase.caseId;
    const body = { subjectLinkId: target.linkId, purpose, fields, reasonCode: reason };
    command.submit(JSON.stringify({ caseId, body }), async key => {
      const grant = unwrap(await operationsApi.POST("/support/cases/{caseId}/data-access-grants", { params: { path: { caseId }, header: { "Idempotency-Key": key } }, body }));
      setAutoReveal(true); setGrantId(grant.grantId); setOpened(n => n + 1);
    }, () => {});
  }
  return <section className="management-workspace"><h2>개인정보 열람</h2>
    {active ? <form className="surface-card management-card" onSubmit={event => { event.preventDefault(); requestGrant(); }}><fieldset className="catalog-fieldset" disabled={locked}><legend>필요한 정보만 선택</legend>
      <SelectField label="열람 대상" value={linkId} onValueChange={value => { setLinkId(value); setFields([]); setGrantId(""); }}><option value="">대상 선택</option>{targets.map(link => <option key={link.linkId} value={link.linkId} disabled={!isSupportSubjectSelectable(link)}>{supportSubjectLabel(link)}</option>)}</SelectField>
      <SelectField label="열람 목적" value={purpose} onValueChange={value => setPurpose(value as Session["purpose"])}>{Object.entries(verificationPurposeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField>
      <p>기본 정보는 10분/3회, 민감 정보는 5분/1회 열람할 수 있습니다.</p>
      {available.map(field => <Checkbox key={field} label={personalFieldLabels[field]} checked={fields.includes(field)} onCheckedChange={checked => setFields(values => checked ? [...values, field] : values.filter(value => value !== field))} />)}
      <SelectField label="열람 요청 사유" value={reason} onValueChange={value => setReason(value as Reason)}>{Object.entries(dataAccessReasonLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField><Button type="submit" loading={command.busy} disabled={!target || !fields.length}>정보 보기</Button>
    </fieldset></form> : !initialGrantId ? <InlineNotice title="진행 중인 상담에서 열람해 주세요" description="현재 담당 상담과 활성 대상 연결이 필요합니다." /> : null}
    ...
    <SupportWorkPicker kind="DATA_ACCESS" caseId={supportCase?.caseId} disabled={locked} onSelect={item => { setAutoReveal(false); setGrantId(item.requestId); setOpened(n => n + 1); }} />
    {grantId ? <GrantInspection key={`${grantId}:${opened}`} grantId={grantId} autoReveal={autoReveal} onBusyChange={setInspectionLocked} /> : null}
  </section>;
}

function GrantInspection({ grantId, autoReveal = false, onBusyChange }: { grantId: string; autoReveal?: boolean; onBusyChange: (value: boolean) => void }) {
  ...
  useEffect(() => { if (read.state.status === "failed" || (grant && (current?.viewerRole !== "REQUESTER" || !["ACTIVE", "CONSUMED"].includes(grant.state)))) clear(); }, [read.state.status, grant?.state, current?.viewerRole, clear]);
  useEffect(() => {
  ...
  async function reveal() {
    if (blocked || inFlight.current || uncertain || !grant || grant.authorizationBasis !== "SUPPORT_DIRECT" || current?.viewerRole !== "REQUESTER" || grant.state !== "ACTIVE" || expired || !selected.length || !selected.every(field => grant.fields.includes(field))) return;
  ...
  useEffect(() => {
    if (autoReveal && !autoStarted.current && current?.viewerRole === "REQUESTER" && grant?.authorizationBasis === "SUPPORT_DIRECT" && grant?.state === "ACTIVE" && !expired) {
      autoStarted.current = true;
      void reveal();
    }
  }, [autoReveal, current, grant?.state, expired]);
  return <div className="management-workspace">{revealError ? <ErrorState error={revealError} /> : null}{uncertain ? <InlineNotice tone="warning" title="원문 열람 응답을 확인하지 못했습니다" description="열람 횟수가 이미 사용되었을 수 있습니다. 원문 응답은 재생되지 않으므로 현재 상태와 감사 이력을 확인해 주세요." /> : null}
    {read.state.status === "loading" ? <LoadingState label="열람 요청과 현재 권한을 확인하는 중" /> : read.state.status === "failed" ? <ErrorState error={read.state.error} retry={read.reload} /> : current && grant ? <article className="surface-card management-card management-workspace"><h3>현재 열람 요청</h3><p className="support-case-reference">열람 요청 ID {grant.grantId}</p><StatusText domain="grant" state={expired ? "EXPIRED" : grant.state} label={grantLabels[expired ? "EXPIRED" : grant.state]} /><dl className="detail-list"><div><dt>상담</dt><dd><ButtonLink variant="ghost" to={`/support/cases/${encodeURIComponent(grant.caseId)}`}>상담 건 열기</ButtonLink></dd></div><div><dt>대상</dt><dd className="support-case-reference">{grant.subjectId}</dd></div><div><dt>목적</dt><dd>{verificationPurposeLabels[grant.purpose]}</dd></div><div><dt>필드 범위</dt><dd>{grant.fields.map(field => personalFieldLabels[field]).join(", ")}</dd></div><div><dt>사용 횟수</dt><dd>{grant.reservedReveals}/{grant.maxReveals}</dd></div><div><dt>요청</dt><dd>{fullDateTime.format(new Date(grant.requestedAt))}</dd></div><div><dt>만료</dt><dd>{grant.expiresAt ? fullDateTime.format(new Date(grant.expiresAt)) : "승인 후 시작"}</dd></div></dl>
      {grant.authorizationBasis === "LEGACY" ? <InlineNotice title="이전 정책의 요청입니다" description="현재 상담에서 대상과 필드를 선택해 새로 열람해 주세요." /> : expired ? <InlineNotice title="열람 기한이 지났습니다" description="현재 상담에서 새 열람을 요청해 주세요." /> : null}
      {grant.authorizationBasis === "SUPPORT_DIRECT" && grant.state === "ACTIVE" && !expired && current.viewerRole === "REQUESTER" && !uncertain ? <fieldset className="catalog-fieldset" disabled={blocked || read.refreshing}><legend>이번에 열람할 정보</legend>{grant.fields.map(field => <Checkbox key={field} label={personalFieldLabels[field]} checked={selected.includes(field)} onCheckedChange={checked => { clear(); setFields(checked ? [...selected, field] : selected.filter(value => value !== field)); }} />)}<Button loading={revealBusy} disabled={!selected.length} onClick={() => void reveal()}>선택한 정보 한시 열람</Button></fieldset> : null}
...
/** Shareable historical grant inspection: URL contains the request identifier only. */
