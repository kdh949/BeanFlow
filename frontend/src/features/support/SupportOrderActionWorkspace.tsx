...
type Case = { caseId: string; state: string; subjectLinks: readonly (SupportSubjectDisplaySource & { linkId?: string; subjectType: string; subjectId: string; relationship: string })[] };
...
const requestStates: Record<Request["state"], string> = { AWAITING_SUPPORT_MANAGER: "상담 관리자 승인 대기", AWAITING_OPERATIONS: "운영 검토 대기", READY_FOR_EXECUTION: "실행 준비", REASSIGNMENT_REQUIRED: "실행 담당자 재배정 필요", REVISION_REQUIRED: "처리 요청 수정 필요", DENIED: "반려", EXPIRED: "만료", STALE: "요청 조건 변경", MANUAL_REVIEW: "수동 확인 필요", EXECUTED: "실행 완료", RESOLUTION_REQUIRED: "수락 후 해결 필요" };
const evaluationReasons: Record<string, string> = { POLICY_ALLOWED: "현재 정책에서 요청할 수 있습니다", POLICY_APPROVAL_REQUIRED: "별도 담당자의 승인이 필요합니다", UNSUPPORTED_TARGET_STATE: "현재 주문 상태에서는 처리할 수 없습니다", CASE_NOT_ELIGIBLE: "진행 중인 상담 건이 필요합니다", TARGET_RELATIONSHIP_MISMATCH: "상담 대상과 주문의 관계가 일치하지 않습니다", MISSING_PERMISSION: "필요한 업무 권한이 없습니다", VERIFICATION_SCOPE_MISMATCH: "업무 처리 목적의 본인확인이 필요합니다", VERIFICATION_PURPOSE_MISMATCH: "상담 해결 목적의 본인확인이 필요합니다", INSUFFICIENT_VERIFICATION: "추가 본인확인이 필요합니다", STALE_TARGET_VERSION: "주문 정보가 변경되었습니다" };

/** Creates typed order changes and inspects the current server-owned approval workflow. */
export function SupportOrderActionWorkspace({ supportCase, initialRequestId, onBusyChange }: { supportCase?: Case; verification?: Verification | null; initialRequestId?: string; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <section className="management-workspace" aria-label="상담 주문 변경">
    ...
    {requestId ? <RequestInspection key={requestId} requestId={requestId} supportCase={supportCase} onBusyChange={setActiveCommand} /> : supportCase ? <CreateOrderRequest onBusyChange={setActiveCommand} supportCase={supportCase} onCreated={id => { setRequestId(id); }} /> : <EmptyState title="상담 건에서 새 요청을 시작해 주세요" description="기존 요청 찾기에서 처리 상태와 실행 담당자를 확인할 수 있습니다." />}
  </section>;
}

export function SupportOrderActionPage() {
  const { requestId } = useParams();
  return <div className="console-page"><PageHeading title="주문 변경 요청과 실행" /><SupportOrderActionWorkspace initialRequestId={requestId} /></div>;
}


function CreateOrderRequest({ supportCase, onCreated, revision, onBusyChange }: { onBusyChange?: (value: boolean) => void; supportCase: Case; verification?: Verification | null; onCreated: (id: string) => void; revision?: Request }) {
  const subjects = supportCase.subjectLinks.filter(link => ["CUSTOMER", "STORE"].includes(link.subjectType) && link.linkId);
  const [subjectLinkId, setSubjectLinkId] = useState(subjects.length === 1 && isSupportSubjectSelectable(subjects[0]!) ? subjects[0]!.linkId! : "");
  ...
  const evaluationExpired = useExpired(evaluation?.expiresAt);
  const active = !["RESOLVED", "CLOSED"].includes(supportCase.state);
  const verified = subjects.some(link => link.linkId === subjectLinkId && isSupportSubjectSelectable(link));
  useEffect(() => { generation.current++; setEvaluation(null); }, [action, orderId, subjectLinkId]);
  ...
  async function evaluate() {
    if (!current || !verified || disabled) return;
    const sequence = ++generation.current; setEvaluating(true); setEvaluation(null); setError(null);
    try { const result = unwrap(await operationsApi.POST("/support/cases/{caseId}/action-evaluations", { params: { path: { caseId: supportCase.caseId } }, body: { action, orderId, expectedTargetVersion: current.version, subjectLinkId } })); if (sequence === generation.current) setEvaluation(result); }
    catch (failure) { if (sequence === generation.current) setError(failure); }
    finally { setEvaluating(false); }
  }
  async function submit() {
    if (!current || !verified || !evaluation || evaluationExpired || evaluation.decision === "DENIED" || disabled) return;
    ...
    try {
      ...
      const body = { expectedTargetVersion: current.version, subjectLinkId, actionPayloadDigest: plan ? await resolutionDigest(orderId, plan) : await orderChangeDigest(action as OrderChangeAction, orderId, reasonCode, slotId), reason: reason.trim(), evidenceDigest: plan?.evidenceDigest ?? await supportDigest(evidence.trim()), ...(plan ? { amountKrw: plan.cashRefundKrw } : {}) };
      if (revision) {
  ...
  return <div className="surface-card management-card management-workspace"><SelectField label="처리 대상" value={subjectLinkId} onValueChange={setSubjectLinkId} disabled={disabled}><option value="">대상 선택</option>{subjects.map(link => <option key={link.linkId} value={link.linkId} disabled={!isSupportSubjectSelectable(link)}>{supportSubjectLabel(link)}</option>)}</SelectField>
    <h3>{revision ? "새 처리 요청 작성" : "새 주문 변경 요청"}</h3>
    ...
    {evaluation ? <InlineNotice tone={evaluation.decision === "DENIED" ? "warning" : "info"} title={evaluation.decision === "DENIED" ? "현재 요청할 수 없습니다" : evaluation.decision === "APPROVAL_REQUIRED" ? "승인 후 실행할 수 있습니다" : "현재 요청할 수 있습니다"} description={`${evaluation.reasonCodes.map(code => evaluationReasons[code] ?? code).join(" · ")}`} /> : null}
    {evaluation && evaluation.decision !== "DENIED" && !evaluationExpired && current ? <form className="operation-form" onSubmit={event => { event.preventDefault(); void submit(); }}>
      ...
      <Button type="submit" disabled={disabled || !reason.trim() || (action === "POST_ACCEPTANCE_RESOLUTION" ? !validResolutionDraft(resolutionDraft) : !evidence.trim()) || (action === "PICKUP_RESCHEDULE" && !slotId)}>{revision ? "새 처리 요청 제출" : "주문 변경 요청 등록"}</Button>
    </form> : null}
...
function RequestInspection({ requestId, supportCase, onBusyChange }: { onBusyChange: (value: boolean) => void; requestId: string; supportCase?: Case; verification?: Verification | null }) {
  ...
  const [reason, setReason] = useState(""); const [assignee, setAssignee] = useState<OperatorSelection | null>(null);
  const [resolutionDraft, setResolutionDraft] = useState<ResolutionDraft>(initialResolutionDraft);
  ...
  const allowed = (name: Workflow["allowedActions"][number]) => !!value && (name === "REVISE" || name === "ADVANCE_RESOLUTION" || !expired) && value.allowedActions.includes(name);
  ...
  function execute() {
    if (!request || blocked || !allowed("EXECUTE") || !direct || !matches) return;
    const common = { revisionNumber: request.revisionNumber, expectedRequestVersion: request.requestVersion, expectedTargetVersion: request.targetVersion };
  ...
  return <section className="surface-card management-card management-workspace" aria-label="현재 주문 변경 요청">
    ...
    {read.state.status === "loading" ? <LoadingState label="현재 처리 요청을 읽는 중" /> : read.state.status === "failed" ? <ErrorState error={read.state.error} retry={read.reload} /> : request && value ? <>
      <h3>{direct ? orderActionLabels[request.action as OrderChangeAction] : "상담 후속 요청"}</h3>
      <p className="support-case-reference">요청 ID {requestId}</p>{request.authorizationBasis === "LEGACY" ? <InlineNotice title="이전 정책의 요청입니다" description="같은 상담에서 새 처리 요청을 작성해 주세요. 기존 처리 이력은 유지됩니다." /> : null}
      <StatusText state={request.state} label={requestStates[request.state]} />
      <dl className="detail-list"><div><dt>처리 요청 / 요청 버전</dt><dd>{request.revisionNumber} / {request.requestVersion}</dd></div><div><dt>대상 주문 버전</dt><dd>{request.targetVersion}</dd></div><div><dt>유효 시간</dt><dd>{shortDateTime.format(new Date(request.expiresAt))}까지</dd></div><div><dt>실행 담당자</dt><dd className="support-case-reference">{request.executorActorId}</dd></div></dl>
      ...
      {expired && request.state !== "EXECUTED" ? <InlineNotice tone="warning" title="처리 요청의 유효 시간이 지났습니다" description="현재 상태를 다시 확인하고 필요한 경우 새 요청을 시작해 주세요." /> : null}
      {(direct || request.action === "POST_ACCEPTANCE_RESOLUTION") && !value.resolutionId && !createdResolutionId && (allowed("EXECUTE") || allowed("DECIDE_SUPPORT_MANAGER")) ? <>
        ...
        <InlineNotice tone={matches ? "info" : "warning"} title={matches ? "선택한 내용이 현재 처리 요청과 일치합니다" : "선택한 내용이 현재 처리 요청과 다릅니다"} description="처리 요청 때 선택한 변경 내용과 증빙을 확인합니다. 내용이 다르면 새 처리 요청이 필요합니다." />
      </> : null}
      {direct && allowed("EXECUTE") ? <form className="operation-form" onSubmit={event => { event.preventDefault(); execute(); }}>
        <Button type="submit" disabled={blocked || !matches}>확인한 주문 변경 실행</Button>
      </form> : null}
      ...
      {allowed("REVISE") && supportCase ? <><Button variant="secondary" disabled={blocked} onClick={() => setRevisionOpen(value => !value)}>처리 요청 수정</Button>{revisionOpen ? <CreateOrderRequest onBusyChange={setRevisionBusy} key={request.revisionNumber} supportCase={supportCase} revision={request} onCreated={() => { setRevisionOpen(false); setMessage("새 처리 요청을 제출했습니다"); read.reload(); }} /> : null}</> : null}
      {!expired && value.allowedActions.length === 0 ? <InlineNotice tone="info" title="현재 담당자가 실행할 명령이 없습니다" description="현재 권한과 요청 상태를 확인해 주세요." /> : null}
    </> : null}
  </section>;
}
