...
const meta = {
  ...
  args: { caseId: id, subjectLinkId: id, orderId: id, disabled: false, onSelect: fn(), onBusyChange: fn() },
...
export const RegisterDistinct: Story = { play: async ({ canvas, args, msw }) => {
  msw.use(http.post("/api/v1/support/cases/:caseId/compensation-incidents", async ({ request }) => { expect(await request.json()).toEqual({ subjectLinkId: id, orderId: id, occurredAt: "2026-09-11T08:00:00.000Z" }); return HttpResponse.json(incident, { status: 201 }); }));
