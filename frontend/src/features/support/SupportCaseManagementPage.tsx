...
import { casePriorityLabels, caseStateLabels, relationshipLabels, subjectLabels } from "./supportCaseLabels";
...
function CaseManagement({ caseId }: { caseId: string }) {
  ...
  const command = useSupportCommand(resource.reload);
  ...
  const selectedLink = current?.subjectLinks.find(link => link.linkId === linkId) ?? current?.subjectLinks[0];
  ...
  function assign() {
    if (!current || !assigneeId.trim() || !assignReason.trim()) return;
    const body = { assigneeId: assigneeId.trim(), expectedVersion: current.version, reason: assignReason.trim() };
    perform("assign", body, async key => unwrap(await operationsApi.POST("/support/cases/{caseId}/assignments", { params: { path: { caseId }, header: { "Idempotency-Key": key } }, body })), "담당자를 배정했습니다", () => { setAssigneeId(""); setAssignReason(""); });
  }
  ...
  function linkSubject() {
    if (!subjectType || !subjectId.trim() || !relationship || !linkReason.trim()) return;
    const body = { subjectType, subjectId: subjectId.trim(), relationship, reason: linkReason.trim() };
    perform("link", body, async key => unwrap(await operationsApi.POST("/support/cases/{caseId}/subject-links", { params: { path: { caseId }, header: { "Idempotency-Key": key } }, body })), "대상을 연결했습니다", () => { setSubjectId(""); setLinkReason(""); });
  }
