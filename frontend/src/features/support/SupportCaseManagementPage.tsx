...
import { casePriorityLabels, caseStateLabels, relationshipLabels, subjectLabels, supportSubjectLabel, isSupportSubjectSelectable } from "./supportCaseLabels";
...
function CaseManagement({ caseId }: { caseId: string }) {
  ...
  const selectedLink = current?.subjectLinks.find(link => (linkId ? link.linkId === linkId : true) && isSupportSubjectSelectable(link));
