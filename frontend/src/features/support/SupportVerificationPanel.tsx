...
export function SupportVerificationPanel({ caseId, links, disabled, onChange, initialActionScope = "PERSONAL_DATA_REVEAL" }: { initialActionScope?: Session["actionScope"]; caseId: string; links: components["schemas"]["SupportSubjectLink"][]; disabled: boolean; onChange: (session: Session | null) => void }) {
  ...
  return <section className="management-workspace"><h2>본인확인</h2>
    ...
    {!eligible.length ? <EmptyState title="본인확인 가능한 대상이 없습니다" description="고객, 매장 또는 배송 대상을 상담 건에 연결해 주세요." /> : <form className="surface-card management-card" onSubmit={e => { e.preventDefault(); create(); }}><fieldset className="catalog-fieldset" disabled={blocked}><legend>새 본인확인</legend><SelectField label="본인확인 대상" value={linkId} onValueChange={setLinkId} required><option value="">대상 선택</option>{eligible.map(link => <option key={link.linkId} value={link.linkId}>{link.subjectType === "CUSTOMER" ? "고객" : link.subjectType === "STORE" ? "매장" : "배송"} · {link.subjectId}</option>)}</SelectField><SelectField label="인증 사용 업무" value={scope} onValueChange={value => setScope(value as Session["actionScope"])}><option value="PERSONAL_DATA_REVEAL">개인정보 열람</option><option value="SUPPORT_ACTION">상담 조치</option></SelectField><SelectField label="본인확인 목적" value={scope === "SUPPORT_ACTION" ? "CASE_RESOLUTION" : purpose} onValueChange={value => setPurpose(value as Session["purpose"])} disabled={scope === "SUPPORT_ACTION"}>{Object.entries(verificationPurposeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField><SelectField label="요청 인증 수준" value={level} onValueChange={value => setLevel(value as typeof level)}><option value="BASIC">기본 · 인증 수단 1개</option><option value="ENHANCED">강화 · 서로 다른 인증 수단 2개</option></SelectField><Button type="submit" loading={command.busy} disabled={!linkId}>본인확인 시작</Button></fieldset></form>}
