...
export function useSupportCommand(onSettled: () => void) {
  ...
  const attempt = useRef<{ key: string; run: (key: string) => Promise<unknown>; onSuccess: () => void } | null>(null);
  ...
  function submit(fingerprint: string, run: (key: string) => Promise<unknown>, onSuccess: () => void) {
    ...
    attempt.current = { key: intent.current.keyFor(fingerprint), run, onSuccess };
    void execute();
  }
