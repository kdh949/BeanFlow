...
export function SupportCaseDirectoryPage() {
  ...
  return <div className="console-page"><PageHeading title="상담 목록" action={<ButtonLink variant="secondary" to="/support">고객 정보 검색</ButtonLink>} /><Tabs value={tab} onValueChange={value => { if (!intakeBusy) setTab(value); }}><TabList label="상담 접수 업무"><Tab value="list">상담 목록</Tab><Tab value="create">새 상담 접수</Tab></TabList><TabPanel value="list"><CaseDirectory /></TabPanel><TabPanel value="create"><CaseIntake onBusy={setIntakeBusy} /></TabPanel></Tabs></div>;
}
function CaseDirectory() {
  const [state, setState] = useState<components["schemas"]["SupportCaseState"] | "">("");
  const [assigneeId, setAssigneeId] = useState("");
  const [query, setQuery] = useState<{ state?: components["schemas"]["SupportCaseState"]; assigneeId?: string }>({});
  ...
  return <section className="management-workspace"><form className="management-card-grid" onSubmit={event => { event.preventDefault(); setCursors([undefined]); setQuery({ state: state || undefined, assigneeId: assigneeId.trim() || undefined }); }}><SelectField label="상담 상태 필터" value={state} onValueChange={value => setState(value as typeof state)}><option value="">모든 상태</option>{Object.entries(caseStateLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField><TextField label="담당자 ID 필터" value={assigneeId} onValueChange={setAssigneeId} /><Button type="submit">상담 검색</Button></form>{resource.state.status === "loading" ? <LoadingState label="상담 목록을 불러오는 중" /> : resource.state.status === "failed" ? <ErrorState error={resource.state.error} retry={resource.reload} /> : <>{resource.state.value.items.length ? <div className="management-card-grid">{resource.state.value.items.map(item => <article key={item.caseId} className="surface-card management-card management-workspace"><h2 className="support-case-reference">{item.caseId}</h2><p><StatusText state={item.state} label={caseStateLabels[item.state]} /> · {casePriorityLabels[item.priority]}</p><dl className="detail-list"><div><dt>담당자</dt><dd className="support-case-reference">{item.assigneeId}</dd></div><div><dt>접수</dt><dd>{fullDateTime.format(new Date(item.openedAt))}</dd></div></dl><ButtonLink to={`/support/cases/${encodeURIComponent(item.caseId)}`}>상담 관리 열기</ButtonLink></article>)}</div> : <EmptyState title="조건에 맞는 상담이 없습니다" description="다른 상태나 담당자로 검색할 수 있습니다." />}<div className="button-row"><Button variant="secondary" disabled={cursors.length === 1} onClick={() => setCursors(value => value.slice(0, -1))}>이전 상담</Button><Button variant="secondary" disabled={!resource.state.value.nextCursor} onClick={() => { if (resource.state.status === "ready") { const next = resource.state.value.nextCursor; if (next) setCursors(value => [...value, next]); } }}>다음 상담</Button></div></>}</section>;
}
function CaseIntake({ onBusy }: { onBusy: (value: boolean) => void }) {
  ...
  const valid = requesterType && reference.trim() && category && reason.trim().length >= (category === "OTHER" ? 3 : 1);
  function create() {
    if (locked || !requesterType || !category || !valid) return;
    const body: components["schemas"]["CreateSupportCaseRequest"] = { requesterType, requesterReference: reference.trim(), category, priority, reason: reason.trim(), ...(externalReference.trim() ? { externalReference: externalReference.trim() } : {}) };
  ...
  return <section className="management-workspace">{created ? <InlineNotice tone="info" announce="polite" title="상담을 접수했습니다" description={`상담 ID ${created.caseId}`} action={<ButtonLink to={`/support/cases/${encodeURIComponent(created.caseId)}`}>접수한 상담 관리</ButtonLink>} /> : null}{command.failure ? <ErrorState error={command.failure} /> : null}{command.pending ? <InlineNotice tone="warning" announce="polite" title="접수 결과를 확인하지 못했습니다" description="입력한 내용으로 같은 접수 요청의 결과를 확인합니다." action={<Button loading={command.busy} onClick={() => void command.retry()}>같은 요청 결과 확인</Button>} /> : null}<form className="surface-card management-card management-workspace" onSubmit={event => { event.preventDefault(); create(); }}><div className="management-card-grid"><SelectField label="요청자 유형" value={requesterType} onValueChange={value => setRequesterType(value as typeof requesterType)} disabled={locked}><option value="">유형 선택</option>{Object.entries(caseRequesterLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</SelectField><TextField label="요청자 참조" value={reference} onValueChange={setReference} maxLength={200} required disabled={locked} description="요청자를 식별하는 참조 값을 입력합니다. 연락처 원문은 고객 정보 검색에서 확인합니다." /><SelectField label="문의 분류" value={category} onValueChange={value => setCategory(value as typeof category)} disabled={locked}><option value="">분류 선택</option>{Object.entries(caseCategoryLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</SelectField><SelectField label="우선순위" value={priority} onValueChange={value => setPriority(value as typeof priority)} disabled={locked}>{Object.entries(casePriorityLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</SelectField><TextField label="외부 접수 참조 (선택)" value={externalReference} onValueChange={setExternalReference} maxLength={200} disabled={locked} /></div><TextAreaField label="접수 사유" value={reason} onValueChange={setReason} maxLength={500} required disabled={locked} /><Button type="submit" loading={command.busy} disabled={locked || !valid}>상담 접수</Button></form></section>;
}
