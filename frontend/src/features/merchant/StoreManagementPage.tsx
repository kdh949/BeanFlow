...
export function StoreManagementPage({ initialWorkspace = "catalog", catalogContent }: { initialWorkspace?: "catalog" | "hours" | "support"; catalogContent?: ReactNode }) {
  ...
  return <div className="console-page store-management-page">
    ...
    <Tabs value={workspace} onValueChange={value => { if (!locked) setWorkspace(value); }}><TabList label="매장 관리 업무 선택"><Tab value="catalog" disabled={locked}>메뉴와 가격</Tab><Tab value="hours" disabled={locked}>영업시간과 이미지</Tab><Tab value="support" disabled={locked}>상담 주문 변경 동의</Tab></TabList>
