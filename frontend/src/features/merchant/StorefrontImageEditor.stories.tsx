...
export const Upload: Story = { beforeEach: () => { present = false; }, play: async ({ canvas }) => { await userEvent.upload(await canvas.findByLabelText("매장 대표 이미지 파일"), new File(["fixture"], "coffee.png", { type: "image/png" })); await waitFor(() => expect(canvas.getByRole("button", { name: "이미지 저장" })).toBeEnabled()); await userEvent.click(canvas.getByRole("button", { name: "이미지 저장" })); await expect(await canvas.findByText("이미지를 저장했습니다.")).toBeVisible(); await expect(await canvas.findByRole("img")).toBeVisible(); } };
export const Delete: Story = { play: async ({ canvas }) => { await waitFor(() => expect(canvas.getByRole("button", { name: "이미지 삭제" })).toBeEnabled()); await userEvent.click(canvas.getByRole("button", { name: "이미지 삭제" })); await userEvent.click(canvas.getByRole("button", { name: "삭제 확인" })); await expect(await canvas.findByText("등록된 이미지가 없습니다.")).toBeVisible(); await expect(canvas.queryByRole("img")).not.toBeInTheDocument(); await expect(canvas.getByText("이미지를 삭제했습니다.")).toBeVisible(); await expect(canvas.queryByRole("alert")).not.toBeInTheDocument(); } };
