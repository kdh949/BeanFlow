...
export function PaymentRepairWorkspace({ caseId }: { caseId?: string }) {
  ...
  function openProposal(id: string, justCreated = false) {
    if (request?.id !== id) { setDecisionReason(""); decisionIntent.current.complete(); }
    setCreated(justCreated); setFailure(null); setRequest({ id });
  }
  async function create() {
    ...
    try {
      ...
      createIntent.current.complete(); setCreated(true); setReason(""); setProposalId(result.proposalId); openProposal(result.proposalId, true);
    } catch (error) { setFailure(error); } finally { setBusy(false); }
  }
  async function decide(decision: "APPROVE" | "REJECT") {
    if (busy || !proposal || !current || proposal.state !== "PENDING_APPROVAL" || current.actor.operatorId === proposal.proposedBy || !decisionReason.trim()) return;
  ...
  return <section className="management-workspace"><h2>누락 환불 복구 제안</h2>
    ...
    <form className="surface-card management-card" onSubmit={event => { event.preventDefault(); openProposal(proposalId.trim()); }}><TextField label="복구 제안 ID" value={proposalId} onValueChange={setProposalId} disabled={busy} required /><Button type="submit" variant="secondary" disabled={busy || !proposalId.trim()}>현재 복구 제안 조회</Button></form>
