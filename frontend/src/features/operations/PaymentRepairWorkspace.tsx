...
export function PaymentRepairWorkspace({ caseId, onLockChange }: { caseId?: string; onLockChange?: (locked: boolean) => void }) {
  ...
  async function create() {
    ...
    if (locked || !target || !reason.trim()) return;
    const body = { reason: reason.trim() };
    await submit({ kind: "create", target, body, key: intent.current.keyFor(JSON.stringify({ kind: "create", target, ...body })) });
  }
  async function decide(decision: "APPROVE" | "REJECT") {
    if (locked || !proposal || !current || expired || proposal.state !== "PENDING_APPROVAL" || current.actor.operatorId === proposal.proposedBy || !decisionReason.trim()) return;
  ...
  async function submit(command: Pending) {
    ...
    try {
      if (command.kind === "create") {
        ...
        setCreated(true); setReason(""); setRequest({ id: result.proposalId });
      ...
      if (unresolved) setPending(command); else { intent.current.complete(); detail.reload(); setRevision(value => value + 1); }
    } finally { submitting.current = false; setBusy(false); }
  }
  return <section className="management-workspace"><h2>누락 환불 복구 제안</h2>
    {!caseId ? <RecoveryCases disabled={locked} revision={revision} selected={selectedCase} onSelect={item => { setSelectedCase(item); setReason(""); setFailure(null); setCreated(false); }} /> : <p>위 주문에서 확인된 환불 처리 정보의 복구를 검토합니다.</p>}
    <form className="surface-card management-card" onSubmit={event => { event.preventDefault(); void create(); }}><fieldset className="catalog-fieldset" disabled={locked}><legend>복구 제안 생성</legend>{selectedCase ? <p>선택한 주문: {selectedCase.order.storeName} · {selectedCase.order.publicReference}</p> : null}<TextField label="복구 제안 사유" value={reason} onValueChange={setReason} maxLength={500} required /><Button type="submit" loading={busy} disabled={!(caseId ?? selectedCase?.caseId) || !reason.trim()}>복구 제안 생성</Button></fieldset></form>
    {created ? <p role="status">복구 제안을 만들었습니다. 다른 담당자의 판정이 필요합니다.</p> : null}
    <RepairProposals caseId={caseId} disabled={locked} revision={revision} onSelect={item => { setSelectedProposal(item); setRequest({ id: item.proposal.proposalId }); setDecisionReason(""); setFailure(null); }} />
...
function RecoveryCases({ disabled, revision, selected, onSelect }: { disabled: boolean; revision: number; selected: CaseItem | null; onSelect: (item: CaseItem) => void }) {
  ...
  return <section className="surface-card management-card"><h3>환불 복구 대상 목록</h3><fieldset className="catalog-fieldset" disabled={disabled}><legend>복구 대상 찾기</legend><SelectField label="복구 대상 상태" value={query.status ?? ""} onValueChange={value => setQuery({ status: value ? value as CaseItem["status"] : undefined })}><option value="">전체</option><option value="OPEN">확인 대기</option><option value="MANUAL_REVIEW">수동 검토</option><option value="RUNNING">처리 중</option><option value="RESOLVED">해결됨</option></SelectField>
    {resource.state.status === "loading" ? <LoadingState label="환불 복구 대상을 불러오는 중" /> : resource.state.status === "failed" ? <ErrorState error={resource.state.error} retry={resource.reload} /> : <>
      {resource.state.value.items.length === 0 ? <EmptyState title="해당 상태의 복구 대상이 없습니다" description="다른 상태를 선택하거나 목록을 새로고침해 주세요." /> : resource.state.value.items.map(item => <article className="management-card" key={item.caseId}><h4>{item.order.storeName}</h4><p>{item.order.publicReference}</p><p>주문 {fullDateTime.format(new Date(item.order.createdAt))}</p><StatusText state={item.status} /><Button variant="secondary" aria-pressed={selected?.caseId === item.caseId} onClick={() => onSelect(item)}>{item.order.storeName} · {item.order.publicReference} 복구 대상 선택</Button></article>)}
      <div className="button-row">{query.cursor ? <Button variant="ghost" onClick={() => setQuery({ status: query.status })}>복구 대상 처음으로</Button> : null}{resource.state.value.nextCursor ? <Button variant="secondary" onClick={() => { if (resource.state.status === "ready") setQuery({ ...query, cursor: resource.state.value.nextCursor ?? undefined }); }}>다음 복구 대상</Button> : null}<Button variant="ghost" onClick={resource.reload}>복구 대상 새로고침</Button></div>
    </>}</fieldset></section>;
}
