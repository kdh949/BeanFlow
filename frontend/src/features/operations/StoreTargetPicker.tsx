...
type StoreRecord = components["schemas"]["OperatorStoreTarget"];
...
export function StoreTargetPicker({ value, onValueChange, disabled = false, purpose }: {
  /** Uses only the selected workflow permission and returns minimal display data. */
  purpose: operations["listStoreTargets"]["parameters"]["query"]["purpose"];
  ...
  async function find(nextCursors: Array<string | undefined>) {
    ...
    try {
      const result = unwrap(await operationsApi.GET("/operations/store-targets", { params: { query: { purpose, query: search.trim() || undefined, cursor: nextCursors.at(-1), limit: 20 } } }));
  ...
  return <fieldset className="catalog-fieldset" disabled={disabled}>
    ...
    {value ? <div className="button-row"><strong>{value.name}</strong><Button type="button" variant="secondary" onClick={() => edit("")}>다른 매장 찾기</Button></div> : <>
      ...
      {state.status === "loading" ? <LoadingState label="매장을 찾는 중" /> : state.status === "failed" ? <ErrorState error={state.error} retry={() => void find(cursors)} /> : state.status === "ready" ? <>
        {state.items.length ? <ul className="menu-authoring-list">{state.items.map(store => <li key={store.storeId}><div><strong>{store.name}</strong></div><Button type="button" variant="secondary" aria-label={`${store.name} 선택`} onClick={() => { ++generation.current; onValueChange(store); }}>선택</Button></li>)}</ul> : <EmptyState title="일치하는 매장이 없습니다" description="매장 이름을 확인하거나 검색어를 비워 전체 목록을 확인해 주세요." />}
