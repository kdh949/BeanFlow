...
export function OperationsRecoveryPage() {
  ...
  return <div className="console-page"><PageHeading title="문제 확인 및 복구" /><Tabs value={workspace} onValueChange={setWorkspace}><TabList label="복구 업무 선택"><Tab value="notification">알림 전달</Tab><Tab value="publication">이벤트 전달</Tab><Tab value="order">주문 후속 처리</Tab><Tab value="repair">환불 복구 승인</Tab></TabList><TabPanel value="notification"><ManualRecoveryWorkspace key="notification" kind="notification" /></TabPanel><TabPanel value="publication"><ManualRecoveryWorkspace key="publication" kind="publication" /></TabPanel><TabPanel value="order"><OrderCompensationWorkspace /></TabPanel><TabPanel value="repair"><PaymentRepairWorkspace /></TabPanel></Tabs></div>;
}
