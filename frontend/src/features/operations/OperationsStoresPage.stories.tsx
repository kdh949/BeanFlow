...
const handlers = [
  http.get("/api/v1/operations/stores", ({ request }) => HttpResponse.json({ items: new URL(request.url).searchParams.has("cursor") ? [{ ...current, storeId: "93000000-0000-4000-8000-000000000002", name: "다음 매장" }] : [current], nextCursor: new URL(request.url).searchParams.has("cursor") ? null : "store-next" })),
...
export const StaleIdentity: Story = { parameters: { msw: { handlers: [http.put("/api/v1/operations/stores/:storeId/identity", () => HttpResponse.json({ code: "STORE_STATE_CONFLICT", message: "다른 운영자가 변경했습니다.", correlationId: "REQ-STORE-STALE" }, { status: 409 })), ...handlers] } }, play: async ({ canvas }) => { await open(canvas); await userEvent.type(canvas.getByLabelText("식별정보 변경 사유"), "정보 정정"); await userEvent.click(canvas.getByRole("button", { name: "식별정보 저장" })); await expect(await canvas.findByText("문의 코드 REQ-STORE-STALE")).toBeVisible(); await expect(canvas.getByRole("button", { name: "현재 식별정보 다시 읽기" })).toBeEnabled(); } };
export const ReadDenied: Story = { parameters: { msw: { handlers: [http.get("/api/v1/operations/stores", () => HttpResponse.json({ code: "ACCESS_DENIED", message: "매장 조회 권한이 없습니다.", correlationId: "REQ-STORE-DENIED" }, { status: 403 }))] } }, play: async ({ canvas }) => { await expect(await canvas.findByText("문의 코드 REQ-STORE-DENIED")).toBeVisible(); await expect(canvas.queryByText("검색 결과가 없습니다.")).not.toBeInTheDocument(); } };
