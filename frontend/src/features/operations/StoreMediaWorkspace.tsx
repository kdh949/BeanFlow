  ...
  return <div className="management-workspace">
    ...
    <section className="surface-card management-card management-workspace"><h3>메뉴 이미지 선택</h3>
      ...
      {menus.state.status === "loading" ? <LoadingState label="메뉴 목록을 불러오는 중" /> : menus.state.status === "failed" ? <ErrorState error={menus.state.error} retry={menus.reload} /> : <>
        {menus.state.value.items.length ? <ul className="menu-authoring-list">{menus.state.value.items.map(menu => <li key={menu.menuId}><div><strong>{menu.name}</strong><p>{menu.lifecycle === "ARCHIVED" ? "보관됨" : "현재 메뉴"}</p><p className="support-case-reference">구분 코드 {menu.menuId}</p></div><Button variant="secondary" aria-label={`${menu.name} · ${menu.menuId} 이미지 관리`} onClick={() => setSelected(menu)}>이미지 관리</Button></li>)}</ul> : <EmptyState title="해당 범위의 메뉴가 없습니다" description="다른 메뉴 범위를 선택해 주세요." />}
    ...
    {selected ? <OperatorStorefrontImageEditor key={selected.menuId} storeId={storeId} menuId={selected.menuId} label={`${selected.name} · ${selected.menuId} 이미지`} /> : null}
  </div>;
}
