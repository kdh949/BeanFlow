...
import { Button, EmptyState, LoadingState, PageHeading, SelectField } from "../../design-system";
...
export function OperationsControlPage() {
  ...
  return <div className="console-page"><PageHeading title="정산 이의제기 운영" />
    {selected ? <><Button variant="ghost" onClick={() => { setSelected(null); list.reload(); }}>이의제기 검색 결과로</Button><DisputeManagementPanel key={selected} audience="operations" load={loadDetail} command={async (operation, body, key) => {
      ...
      <form className="surface-card management-card" onSubmit={event => { event.preventDefault(); if (!store) return; setCursors([undefined]); setQuery({ storeId: store.storeId, state: filter || undefined }); }}><StoreTargetPicker purpose="DISPUTE" value={store} onValueChange={next => { setStore(next); setQuery(null); setSelected(null); setCursors([undefined]); }} /><SelectField label="이의제기 상태" value={filter} onValueChange={value => setFilter(value as typeof filter)}>{states.map(state => <option key={state.value} value={state.value}>{state.label}</option>)}</SelectField><Button type="submit" disabled={!store}>이의제기 조회</Button></form>
      {!query ? null : list.state.status === "loading" ? <LoadingState label="이의제기를 불러오는 중" /> : list.state.status === "failed" ? <ErrorState error={list.state.error} retry={list.reload} /> : list.state.value ? <>
