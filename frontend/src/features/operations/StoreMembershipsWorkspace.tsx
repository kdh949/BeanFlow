...
function MembershipManagement({ storeId }: { storeId: string }) {
  ...
  return <section className="management-workspace"><h3>점주·직원 소속</h3>
    ...
    {list.state.status === "loading" ? <LoadingState label="매장 소속을 불러오는 중" /> : list.state.status === "failed" ? <ErrorState error={list.state.error} retry={list.reload} /> : <>
      ...
      {list.state.value.items.length ? <ul className="menu-authoring-list">{list.state.value.items.map(item => <li key={item.membershipId}><div><p><strong>{item.accountDisplayName}</strong> · {item.accountLoginId}</p><p>{item.role === "OWNER" ? "점주" : "직원"}</p><p>{item.status === "ACTIVE" ? "활성" : "철회됨"}</p></div><Button variant="secondary" aria-label={`${item.accountDisplayName} (${item.accountLoginId}) 소속 변경`} onClick={() => { setSelected(item.accountId); setAdding(false); setNotice(""); }}>변경</Button></li>)}</ul> : <EmptyState title="등록된 소속이 없습니다" description="기존 계정을 조회해 점주 또는 직원 소속을 추가할 수 있습니다." />}
