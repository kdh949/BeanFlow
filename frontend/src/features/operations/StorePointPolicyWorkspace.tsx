...
export function StorePointPolicyWorkspace({ storeId }: { storeId: string }) {
  ...
  return <section className="management-workspace"><h3>매장 포인트 정책</h3><p className="support-case-reference">{storeId}</p>
    <div className="button-row"><SelectField label="매장 포인트 조회 사유" value={reason} onValueChange={setReason}><option value="">조회 목적 선택</option><option value="POLICY_CHANGE_REVIEW">정책 변경 전 확인</option><option value="POLICY_AUDIT_REVIEW">정책 감사</option></SelectField><Button variant="secondary" disabled={!reason} onClick={() => { setRequest({ reason }); setSaved(false); }}>현재 매장 포인트 정책 조회</Button></div>
    ...
    {current.state.status === "loading" ? <LoadingState label="현재 매장 포인트 정책을 불러오는 중" /> : current.state.status === "failed" ? <ErrorState error={current.state.error} retry={current.reload} /> : current.state.value ? <>
      <section className="surface-card management-card"><h4>{sources[current.state.value.selectionSource]}</h4><p>실제 적립률 {(current.state.value.effectivePolicy.accrualRateBps / 100).toFixed(2)}%</p><dl className="detail-list"><div><dt>적용 버전</dt><dd>{current.state.value.effectivePolicy.policyVersionId}</dd></div><div><dt>반올림</dt><dd>{current.state.value.effectivePolicy.roundingMode === "FLOOR" ? "버림" : "반올림"}</dd></div><div><dt>비용 주체</dt><dd>{current.state.value.effectivePolicy.issuerType} · {current.state.value.effectivePolicy.issuerReference}</dd></div><div><dt>유효기간</dt><dd>{current.state.value.effectivePolicy.validityDays}일 · {current.state.value.effectivePolicy.expiryRule === "EXACT_DURATION_FROM_COMPLETION" ? "정확한 시간" : "서울 달력일"}</dd></div></dl></section>
      <PolicyForm key={`${current.state.value.explicitHead?.policyVersionId ?? "none"}:${current.state.value.effectivePolicy.policyVersionId}`} storeId={storeId} current={current.state.value} onSaved={() => { setSaved(true); setRevision(value => value + 1); current.reload(); }} onRefresh={current.reload} />
    </> : null}
    <PointPolicyHistory key={revision} storeId={storeId} />
  </section>;
}
function PolicyForm({ storeId, current, onSaved, onRefresh }: { storeId: string; current: Policy; onSaved: () => void; onRefresh: () => void }) {
  ...
  const [issuerType, setIssuerType] = useState(current.effectivePolicy.issuerType);
  const [issuerReference, setIssuerReference] = useState(current.effectivePolicy.issuerReference);
  ...
  async function save() {
    ...
    const body = state === "INHERIT_GLOBAL" ? { ...common, state } : { ...common, state, accrualRateBps: Math.round(Number(rate) * 100), roundingMode: rounding, issuerType, issuerReference: issuerReference.trim(), expiryRule: expiry, validityDays: Number(days) };
    setBusy(true); setFailure(null);
    try { unwrap(await operationsApi.PATCH("/operations/policies/ordinary-point-accrual/stores/{storeId}", { params: { path: { storeId }, header: { "Idempotency-Key": intent.current.keyFor(JSON.stringify({ storeId, ...body })) } }, body })); intent.current.complete(); onSaved(); }
    catch (error) { setFailure(error); } finally { setBusy(false); }
  }
  return <form className="surface-card management-card" onSubmit={event => { event.preventDefault(); void save(); }}><fieldset className="catalog-fieldset" disabled={busy}><legend>매장 정책 변경</legend>
    ...
    {state === "OVERRIDE" ? <div className="management-card-grid">
      ...
      <SelectField label="포인트 비용 주체" value={issuerType} onValueChange={value => setIssuerType(value as Snapshot["issuerType"])}><option value="PLATFORM">플랫폼</option><option value="BRAND">브랜드</option><option value="STORE">매장</option></SelectField>
      <TextField label="포인트 비용 주체 식별값" value={issuerReference} onValueChange={setIssuerReference} maxLength={240} required />
    ...
    <div className="button-row"><Button type="submit" loading={busy} disabled={!reason.trim()}>매장 포인트 정책 저장</Button><Button type="button" variant="secondary" onClick={onRefresh}>현재 매장 정책 다시 읽기</Button></div>
  </fieldset></form>;
}
