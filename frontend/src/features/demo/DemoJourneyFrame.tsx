...
export function DemoJourneyFrame({ surface, children }: { surface: "store" | "customer" | "none"; children: ReactNode }) {
  ...
  return <div className={`demo-frame demo-frame--${surface}`}>
    ...
    {demo.error ? <InlineNotice tone="warning" announce="assertive" title="체험 상태 확인이 필요해요" description={demo.error} action={<Button variant="secondary" onClick={() => void demo.retry()}>상태 다시 확인</Button>} /> : null}
