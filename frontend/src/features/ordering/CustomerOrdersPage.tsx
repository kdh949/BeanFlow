...
export function CustomerOrdersPage() {
  ...
  return <div className="customer-page customer-orders-page"><PageHeading title="주문" description="진행 중인 픽업과 지난 주문을 한곳에서 확인하세요." /><div className="order-tabs" role="tablist" aria-label="주문 상태"><button type="button" role="tab" aria-selected={status === "ACTIVE"} onClick={() => update({ status: "ACTIVE" })}>진행 중</button><button type="button" role="tab" aria-selected={status === "PAST"} onClick={() => update({ status: "PAST" })}>지난 주문</button></div><div className="order-date-filter surface-card"><CalendarDays size={18} /><label htmlFor="customer-orders-from">조회 시작일</label><input id="customer-orders-from" type="date" value={from} max={to} onChange={(event) => update({ from: event.target.value })} /><label htmlFor="customer-orders-to">조회 종료일</label><input id="customer-orders-to" type="date" value={to} min={from} onChange={(event) => update({ to: event.target.value })} /></div>{!page && !error ? <LoadingState label="주문을 불러오는 중" /> : null}{error ? <ErrorState error={error} retry={() => void load()} /> : null}{page?.items.length === 0 ? <EmptyState title={status === "ACTIVE" ? "진행 중인 주문이 없어요" : "이 기간의 주문이 없어요"} description={status === "ACTIVE" ? "새 주문을 시작하면 픽업 상태가 여기에 표시됩니다." : "조회 기간을 넓혀 다시 확인해 보세요."} action={<ButtonLink to="/app/stores">매장 찾기</ButtonLink>} /> : null}{page?.items.length ? <section className="customer-order-list" aria-label={status === "ACTIVE" ? "진행 중인 주문" : "지난 주문"}>{page.items.map((order) => <OrderRow key={order.orderReference} order={order} active={status === "ACTIVE"} />)}</section> : null}{page?.page.nextCursor ? <Button block variant="secondary" loading={loadingMore} onClick={() => void load(page.page.nextCursor, true)}><RefreshCw size={16} />{loadingMore ? "더 불러오는 중" : "주문 더 보기"}</Button> : null}</div>;
}

function OrderRow({ order, active }: { order: CustomerOrderSummary; active: boolean }) {
  return <Link className={`customer-order-row surface-card ${active ? "is-active" : ""}`} to={`/app/orders/${order.orderReference}`}><div className="customer-order-row-head"><StatusText state={order.status} /><span>{order.storeName}</span></div>{active ? <strong className="pickup-number">{order.pickupNumber}</strong> : null}<div className="customer-order-row-body"><div><strong>{order.itemSummary}</strong><span>{shortDateTime.format(new Date(order.orderedAt))} · {order.pickupNumber}</span></div><div><strong>{won.format(order.totalAmountKrw)}</strong><ArrowRight size={18} /></div></div></Link>;
}
