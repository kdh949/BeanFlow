...
export function CustomerOrdersPage() {
  ...
  return <div className="customer-page customer-orders-page"><PageHeading title="주문" description="진행 중인 픽업과 지난 주문을 한곳에서 확인하세요." /><Tabs value={status} onValueChange={(value) => update({ status: value as CustomerOrderStatus })}><TabList label="주문 상태"><Tab value="ACTIVE">진행 중</Tab><Tab value="PAST">지난 주문</Tab></TabList><div className="order-date-filter surface-card"><CalendarDays size={18} /><TextField label="조회 시작일" id="customer-orders-from" type="date" value={from} max={to} onValueChange={(value) => update({ from: value })} /><TextField label="조회 종료일" id="customer-orders-to" type="date" value={to} min={from} onValueChange={(value) => update({ to: value })} /></div><TabPanel value="ACTIVE">{status === "ACTIVE" ? content : null}</TabPanel><TabPanel value="PAST">{status === "PAST" ? content : null}</TabPanel></Tabs></div>;
}
